...
public class SuperTieBreak implements Serializable {
    ...
    public void setEqualTo(SuperTieBreak superTieBreak) {
        ...
        this.fastFourMatch = superTieBreak.isFastFourMatch();
    }
    ...
    GameOrTieBreakEventResult updateForPointPlayed(boolean pointWonByA) {
        ...
        if (this.fastFourMatch) {
            if (pointsA == 4 && pointsB == 4) {
                onServeSideNow = TeamId.UNKNOWN;
            } else if (!GCMath.isOdd(pointsA + pointsB))
        ...
        if (this.fastFourMatch && pointsA == 4 && pointsB == 4) {
            onServePlayerTeamANow = 0;
            onServePlayerTeamBNow = 0;
        } else {
