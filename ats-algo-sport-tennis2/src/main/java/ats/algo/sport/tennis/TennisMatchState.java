...
import ats.algo.genericsupportfunctions.Tools;
...
public class TennisMatchState extends AlgoMatchState {
    ...
    private Map<TeamId, Integer> doubleFaultCounterMap;
    private PairOfIntegers[] acesInSetN;
    ...
    private boolean inWimblendon;
    private final int nGamesInFinalSetWimblendon = 12;
    ...
    private TennisMatchIncidentResult mostRecentMatchIncidentResult; // the
                                                                     // state
    private AbandonMatchIncident abandonMatchIncident;

    public int getnGamesInFinalSet() {
        return nGamesInFinalSetWimblendon;
    }

    public boolean isInWimblendon() {
        return inWimblendon;
    }

    public void setInWimblendon(boolean inWimblendon) {
        this.inWimblendon = inWimblendon;
    }
    ...
    public PairOfIntegers[] getAcesInSetN() {
        return acesInSetN;
    }

    public void setAcesInSetN(PairOfIntegers[] acesInSetN) {
        this.acesInSetN = acesInSetN;
    }
    ...
    public AbandonMatchIncident getAbandonMatchIncident() {
        return abandonMatchIncident;
    }

    public void setAbandonMatchIncident(AbandonMatchIncident abandonMatchIncident) {
        this.abandonMatchIncident = abandonMatchIncident;
    }
     ...
     * @return
     */
    public PairOfIntegers getAcesInSetN(int i) {
        if (i < matchFormat.getSetsPerMatch())
            return acesInSetN[i];
        else
            return null;
     ...
     * @param acesB
     */
    void setAcesInSetN(int setNo, int acesA, int acesB) {
        acesInSetN[setNo].A = acesA;
        acesInSetN[setNo].B = acesB;
    }
    ...
    public boolean isGameMayBePlayed(int n) {
        if (inWimblendon) {
            if (inFinalSet)
                return n >= getGameNo() && (inAdvantageSet() || n <= nGamesInFinalSetWimblendon * 2);
        }
    ...
    public TennisMatchState(MatchFormat format) {
        ...
        if (!matchFormat.getSex().equals(TennisMatchFormat.Sex.MIXED)) {
            ...
            genderMap = new EnumMap<PlayerId, GenderId>(PlayerId.class);
            genderMap.put(PlayerId.A1, GenderId.MALE);
            genderMap.put(PlayerId.B1, GenderId.MALE);
            genderMap.put(PlayerId.A2, GenderId.FEMALE);
            genderMap.put(PlayerId.B2, GenderId.FEMALE);
        }
        this.tournamentLevel = matchFormat.getTournamentLevel();
        ...
        if (this.tournamentLevel == TournamentLevel.FAST4) {
            nGamesInMatch = 4;
            // nSetsInMatch = 3;
            game.setFastFourMatch(true);
            tieBreak.setFastFourMatch(true);
            superTieBreak.setFastFourMatch(true);
        }
        inWimblendon = false;
        this.matchFormat = matchFormat;
        this.doublesMatch = matchFormat.isDoublesMatch();
        this.doubleFaultCounterMap = new HashMap<>();
        this.doubleFaultCounterMap.put(TeamId.A, 0);
        this.doubleFaultCounterMap.put(TeamId.B, 0);
        // if (this.doublesMatch) {
        // this.doubleFaultCounterMap.put(PlayerId.A2, 0);
        // this.doubleFaultCounterMap.put(PlayerId.B2, 0);
        // }
        ...
        acesInSetN = new PairOfIntegers[nSetsInMatch];
        ...
        for (int i = 0; i < nSetsInMatch; i++) {
            ...
            acesInSetN[i] = new PairOfIntegers();
        }
        ...
        switch (matchFormat.getFinalSetType()) {
            ...
            case AUSTRALIAN_OPEN:
                tieBreak.setnPointsInFinalSetTieBreakGame(10);
                break;
            default:
                break;
        ...
        game.setStoreLastGameScore((StoreLastGameScore & Serializable) (int a, int b) -> this.storeLastGameScore(a, b));
    ...
    public Map<TeamId, Integer> getDoubleFaultCounterMap() {
        return doubleFaultCounterMap;
    }

    public void setDoubleFaultCounterMap(Map<TeamId, Integer> doubleFaultCounterMap) {
        this.doubleFaultCounterMap = doubleFaultCounterMap;
    }
    ...
    boolean checkTieBreaks(int setsA, int setsB, int gamesA, int gamesB, int pointsA, int pointsB,
        ...
        if (inFinalSetNow) {
            if (!finalSetSuperTieBreak) {
                if (inWimblendon)
                    out = gamesA == nGamesInFinalSetWimblendon && gamesB == nGamesInFinalSetWimblendon
                                    && tieBreakinFinalSet && finalSetSuperTBNo == 0 && (pointsA == 7 || pointsB == 7);
                else
                    out = gamesA == nGamesInMatch && gamesB == nGamesInMatch && tieBreakinFinalSet
                                    && finalSetSuperTBNo == 0 && (pointsA == 7 || pointsB == 7);
            } else {
    ...
    public void setScore(int setsA, int setsB, int gamesA, int gamesB, TeamId onServeSideNow, int onServePlayerTeamANow,
        ...
        if (inFinalSet) {
            if (!finalSetSuperTieBreak) {
                if (inWimblendon)
                    inTieBreak = gamesA == nGamesInFinalSetWimblendon && gamesB == nGamesInFinalSetWimblendon
                                    && tieBreakinFinalSet && finalSetSuperTBNo == 0;
                else
    ...
    public void setEqualTo(AlgoMatchState ms) {
        ...
        this.inWimblendon = ts.inWimblendon;
        ...
        this.tournamentLevel = ts.getTournamentLevel();
        ...
        for (int i = 0; i < matchFormat.getSetsPerMatch(); i++) {
            ...
            if (i < ts.getAcesScoreMap().size()) {
                PairOfIntegers aces = ts.getAcesInSetN(i);
                this.acesInSetN[i].A = aces.A;
                this.acesInSetN[i].B = aces.B;
            } else {
                PairOfIntegers aces = PairOfIntegers.generateFromString("0-0");
                this.acesInSetN[i].A = aces.A;
                this.acesInSetN[i].B = aces.B;
            }
        ...
        this.setDoubleFaultCounterMap(Tools.hashMapClone(ts.getDoubleFaultCounterMap()));

        this.previousGameScore = PairOfIntegers
                        .generateFromString(ts.getPreviousGameScore().A + "-" + ts.getPreviousGameScore().B);

        this.game.setStoreLastGameScore(
                        (StoreLastGameScore & Serializable) (int a, int b) -> this.storeLastGameScore(a, b));
        ...
        this.setAbandonMatchIncident(ts.getAbandonMatchIncident());
    }
    ...
    public MatchIncidentResult updateStateForIncident(MatchIncident matchIncident,
        ...
        if ((matchIncident instanceof AbandonMatchIncident)) {
            abandonMatchIncident = (AbandonMatchIncident) matchIncident;
        ...
        if (doubletennisMatchIncident.getIncidentSubType() == TennisMatchIncidentType.POINT_WON)
            if (doubletennisMatchIncident.getPointResult() == TennisPointResult.DOUBLE_FAULT) {
                TeamId teamId = (TeamId) getItemOpposit(doubletennisMatchIncident.getPointWinner());
                int counter = this.doubleFaultCounterMap.get(teamId);
                this.doubleFaultCounterMap.put(teamId, counter + 1);
            }
        ...
        if (stateFromFeed != null) {
            ...
            if (isDataFeedStateTieBreakResult) {
                ...
                if (shouldUpdateSetWonAtTieBreak(stateFromFeed.getPointsA(), stateFromFeed.getPointsB(),
                    ...
                    if (sA <= this.getSetsA())
                        sA++;
                    stateFromFeed = new TennisMatchStateFromFeed(sA, sB, 0, 0, 0, 0, false, TeamId.A);
                    ...
                    if (sB <= this.getSetsB())
                        sB++;
                    stateFromFeed = new TennisMatchStateFromFeed(sA, sB, 0, 0, 0, 0, false, TeamId.B);
                    if ((sA == matchWinningSetScore) || (sB == matchWinningSetScore))
        ...
        if (doubletennisMatchIncident.getIncidentSubType().equals(TennisMatchIncidentType.INJURY_STATUS_REVERSE)) {
            ...
            if (doubletennisMatchIncident.getIncidentSubType() == TennisMatchIncidentType.MATCH_STARTING) {
                ...
                if (doublesMatch) {
                    ...
                    if (onServePlayerInFirstGameOfSet == PlayerId.UNKNOWN) {
                        ...
                        if (this.getSetNo() == 1 && this.getGameNo() == 1 && this.getPointNo() == 1) {
                            if (doubletennisMatchIncident.getServerSideAtStartOfCurrentGame() == TeamId.A) {
                                if (doubletennisMatchIncident.getServerPlayerAtStartOfCurrentGame() == 1) {
                                    this.onServePlayerInFirstGameOfSet = PlayerId.A1;
                                    this.onServePlayerInSecondGameOfSet = PlayerId.UNKNOWN;
                                }
                                if (doubletennisMatchIncident.getServerPlayerAtStartOfCurrentGame() == 2) {
                                    this.onServePlayerInFirstGameOfSet = PlayerId.A2;
                                    this.onServePlayerInSecondGameOfSet = PlayerId.UNKNOWN;
                            ...
                            this.startOfMatchServer = onServePlayerInFirstGameOfSet;
                        }
                    ...
                    /* only chance to overwrite the start of match server */
                    if (this.getSetNo() == 1 && this.getGameNo() == 1 && this.getGameNo() == 1) {
                        this.startOfMatchServer = onServePlayerInFirstGameOfSet;
                    }
            ...
            if (doubletennisMatchIncident.getIncidentSubType() == TennisMatchIncidentType.PENALTY_POINT_WON
                ...
                boolean pointWonByA = doubletennisMatchIncident.getPointWinner() == TeamId.A;

                if (doubletennisMatchIncident.getPointResult() != null) {
                    if (doubletennisMatchIncident.getPointResult() == TennisPointResult.ACE) {

                        int setNo = setsA + setsB;
                        if (pointWonByA) {
                            acesInSetN[setNo].A++;
                        } else if (doubletennisMatchIncident.getPointWinner() == TeamId.B) {
                            acesInSetN[setNo].B++;
                        }
                    }
                }
                ...
                if (inTieBreak) {
                    if (!inSuperTieBreakCurrently) {
                        ...
                        gameEventResult = tieBreak.updateForPointPlayed(pointWonByA, inFinalSet);
                        ...
                        // if(superTieBreak.pointsA==4&&superTieBreak.pointsB==4) {
            ...
            switch (gameEventResult) {
                ...
                case SUPERTIEBREAKSETWONB:
                    lastGamePlayedOutcome = TeamId.A;
                    doubletennisMatchIncidentResult = new TennisMatchIncidentResult(onServeSideNow == TeamId.B, true,
                                    onServePlayerNow, TennisMatchIncidentResultType.GAMEWONB);
                    // lastSetWonB = setsB;
                    gamesB++;
                    setNo = setsA + setsB;
    ...
    private void resetTieBreakStatus(TennisMatchStateFromFeed stateFromFeed) {
        ...
        if (fGetA < this.matchWinningSetScore - 1 && fGetB < this.matchWinningSetScore - 1) {// not
            ...
            if (this.finalSetSuperTieBreak) { // super tie break mode
                ...
                if (inWimblendon) {
                    if (fGameA == nGamesInFinalSetWimblendon && fGameB == nGamesInFinalSetWimblendon) {
                        inTieBreak = true;
                        inSuperTieBreak = false;
                    }
                } else if (fGameA == nGamesInMatch && fGameB == nGamesInMatch) {
                    inTieBreak = true;
                    inSuperTieBreak = false;
                }
            }
    ...
    private <e> Enum<?> getItemOpposit(Enum<?> genderId) {
        ...
        } else if (genderId.getClass() == TeamId.class) {
            if (genderId == TeamId.A)
                return TeamId.B;
            else if (genderId == TeamId.B)
                return TeamId.A;
            else
                return TeamId.UNKNOWN;
        }
        ...
        if (matchIncident.getIncidentSubType() == TennisMatchIncidentType.POINT_WON) {
            if (doublesMatch) {
                if (this.getOnServeTeamNow() == TeamId.UNKNOWN) {
                    int servingPlayerNow =
                                    decideServingPlayerFromPoints(matchIncident.getServerSideAtStartOfCurrentGame(),
                                                    matchIncident.getServerPlayerAtStartOfCurrentGame());
                    TeamId servingTeamNow = decideServingTeamFromPoints(matchIncident);
                    this.setOnServeSideNow(servingTeamNow);
                    if (this.getOnServePlayerNow() == PlayerId.UNKNOWN) {
                    ...
                    if (this.getOnServePlayerNow() == PlayerId.UNKNOWN) {
                        if (this.getOnServeNow().equals(TeamId.A)) {
                            this.setOnServePlayerTeamANow(1);
                        } else if (this.getOnServeNow().equals(TeamId.B)) {
    ...
    private TeamId decideServingTeamFromPoints(TennisMatchIncident incident) {
        TeamId serverSideAtStartOfMatch = incident.getServerSideAtStartOfMatch();
        if (serverSideAtStartOfMatch == null || serverSideAtStartOfMatch.equals(TeamId.UNKNOWN)) {
            if (getGameNo() % 2 != 0) {
                if (getOnServeNow().equals(TeamId.A)) {
                    serverSideAtStartOfMatch = TeamId.A;
                } else if (getOnServeNow().equals(TeamId.B)) {
                    serverSideAtStartOfMatch = TeamId.B;
                }
            } else {
                if (getOnServeNow().equals(TeamId.A)) {
                    serverSideAtStartOfMatch = TeamId.B;
        ...
        if (serverSideAtStartOfMatch == null || serverSideAtStartOfMatch.equals(TeamId.UNKNOWN)) {
            if (onServePlayerInFirstGameOfSet.equals(PlayerId.A1)
                            || onServePlayerInFirstGameOfSet.equals(PlayerId.A2)) {
                serverSideAtStartOfMatch = TeamId.A;
            } else if (onServePlayerInFirstGameOfSet.equals(PlayerId.B1)
                            || onServePlayerInFirstGameOfSet.equals(PlayerId.B2)) {
                serverSideAtStartOfMatch = TeamId.B;
            } else {
                throw new IllegalArgumentException("BULLSHIT - no idea who is serving");
            }
    ...
    private void updateTieBreakStatus(TennisMatchIncidentResult doubletennisMatchIncidentResult) {
        ...
        if (inFinalSet)
            if (finalSetSuperTieBreak) {
                inSuperTieBreak = true;
                inTieBreak = true;
            } else if (tournamentLevel == TournamentLevel.FAST4) {
            ...
            if (!tieBreakinRegularSet) {
                ...
                if (tournamentLevel == TournamentLevel.FAST4) {
                    if ((gamesA == 3) && (gamesB == 3)) {
                        inTieBreak = true;
                        inSuperTieBreak = false;
                    } else {
    ...
    private boolean isSetWinningGameScore(int gamesWinner, int gamesLoser) {
        ...
        if (inAdvantageSet())
            ...
            if (inWimblendon && inFinalSet) {
                return (gamesWinner == (nGamesInFinalSetWimblendon + 1))
                                || (gamesWinner >= nGamesInMatch && gamesLoser <= (nGamesInMatch - 2));
            }
            return (gamesWinner == (nGamesInMatch + 1))
                            || (gamesWinner == nGamesInMatch && gamesLoser <= (nGamesInMatch - 2));
        } else if (nGamesInMatch == 4 && this.tournamentLevel == TournamentLevel.FAST4)// fast four match
        {
            return gamesWinner == nGamesInMatch;
        } else
            return (gamesWinner == (nGamesInMatch + 1));
    }
    ...
    public LinkedHashMap<String, String> getAsMap() {
        ...
        if (matchFormat != null) { // can currently null when deserialised from
            ...
            for (int i = 0; i < matchFormat.getSetsPerMatch() - 1; i++) {
                PairOfIntegers ace = acesInSetN[i];
                map.put(String.format("Ace score in set no %d", i + 1), String.format("%d-%d", ace.A, ace.B));
            }
        }
    ...
    public boolean isMatchCompleted() {
        ...
        } else if (getMostRecentMatchIncidentResult() != null
                        && getMostRecentMatchIncidentResult().getTennisMatchIncidentResultType() != null
                        && (getMostRecentMatchIncidentResult().getTennisMatchIncidentResultType()
                                        .equals(TennisMatchIncidentResultType.MATCHABANDONED))) {
            if (getAbandonMatchIncident() != null && getAbandonMatchIncident().getIncidentSubType()
                            .equals(AbandonMatchIncidentType.SCOUT_ABANDONED)) {
                return false;
            }
            return true;
        } else {
    ...
    /**
     * returns total no of aces scored by player A in the match
     *
     * @return
     */
    public int getTotalAcesA() {
        int nSets = acesInSetN.length;
        int nAces = 0;
        for (int i = 0; i < nSets; i++) {
            nAces += acesInSetN[i].A;
        }
        return nAces;
    }

    /**
     * returns total no of aces scored by player B in the match
     *
     * @return
     */
    public int getTotalAcesB() {
        int nSets = acesInSetN.length;
        int nAces = 0;
        for (int i = 0; i < nSets; i++) {
            nAces += acesInSetN[i].B;
        }
        return nAces;
    }
    ...
    private LinkedHashMap<String, PairOfIntegers> getAcesScoreMap() {
        LinkedHashMap<String, PairOfIntegers> scoreMap = new LinkedHashMap<String, PairOfIntegers>();
        for (int i = 0; i < getSetNo(); i++) {
            if (i < this.matchFormat.getSetsPerMatch()) {
                scoreMap.put("acesInSet" + Integer.toString(i + 1), acesInSetN[i]);
            } else {
                continue;
            }
        }
    ...
    public TennisSimpleMatchState generateSimpleMatchState() {
        TennisSimpleMatchState simpleMatchState = new TennisSimpleMatchState(isPreMatch(), isMatchCompleted(),
                        ...
                        getSetNo(), isInTieBreak(), isInSuperTieBreak(), getGamesScoreMap(), getTotalAcesA(),
                        getTotalAcesB(), getTotalDoubleFault(TeamId.A), getTotalDoubleFault(TeamId.B));
        return simpleMatchState;
    }

    @JsonIgnore
    int getTotalDoubleFault(TeamId a) {
        int num = 0;
        if (this.doubleFaultCounterMap == null)
            return 0;
        else {
            num = this.doubleFaultCounterMap.get(a);
        }
        return num;
    }

    public void setMatchStateEqualToFeedState(AlgoMatchState matchState, TennisMatchStateFromFeed matchIncidentState) {
    ...
    public AlgoMatchState generateMatchStateForMatchResult(MatchResultMap matchManualResult) {
        ...
        int nSets = ((TennisMatchFormat) this.getMatchFormat()).getSetsPerMatch();
        ...
        int gamesA = 0;
        int gamesB = 0;
        TennisMatchState endMatchState = (TennisMatchState) this.copy();
        for (int i = 0; i < nSets; i++) {
            int nA = setScore[i].A;
            int nB = setScore[i].B;
    ...
    @Override
    public void setIncidentTeamId(MatchIncident matchIncident) {
        if (matchIncident == null) {
            setIncidentTeamId(TeamId.UNKNOWN.toString());
            return;
        }
        String teamId = null;
        TennisMatchIncident tennisMatchIncident = (TennisMatchIncident) matchIncident;
        switch (tennisMatchIncident.getIncidentSubType()) {
            case POINT_WON:
                teamId = tennisMatchIncident.getPointWinner().toString();
                break;
            case FAULT:
                teamId = getOnServeNow().toString();
                break;
            case MATCH_STARTING:
                teamId = tennisMatchIncident.getServerSideAtStartOfMatch().toString();
                break;
            case SERVING_ORDER:
                teamId = tennisMatchIncident.getServerSideAtStartOfCurrentGame().toString();
                break;
            default:
                break;
        }
        if (teamId != null) {
            setIncidentTeamId(teamId);
        }
    }
}
