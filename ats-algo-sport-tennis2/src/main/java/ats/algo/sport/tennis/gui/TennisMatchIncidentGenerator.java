...
public class TennisMatchIncidentGenerator extends MatchIncidentGenerator {
    ...
    public TennisMatchIncidentGenerator() {
        ...
        standardOptions.put("RETIREMENT", AbandonMatchIncidentType.RETIREMENT);
        standardOptions.put("WALKOVER", AbandonMatchIncidentType.WALKOVER);
        standardOptions.put("BET_STOP", DatafeedMatchIncidentType.BET_STOP);
        standardOptions.put("BET_START", DatafeedMatchIncidentType.BET_START);
        standardOptions.put("SCOUT_ABANDONED", AbandonMatchIncidentType.SCOUT_ABANDONED);
        standardOptions.put("CANCELLATION", AbandonMatchIncidentType.CANCELLATION);
    }
    ...
    private void addStandardGuiOptionsPrematch(GuiDataComponent guiDataComponent) {
        ...
        for (String optionName : standardOptions.keySet())
            if (optionName.equals("MATCH_STARTING") || optionName.equals("SERVING_ORDER")
                            || optionName.equals("CANCELLATION") || optionName.equals("WALKOVER"))
    ...
    public MatchIncident generateMatchIncident(GuiIncidentResponse guiResponse) {
        ...
        if (optionName.contains("RETIREMENT")) {
            return new AbandonMatchIncident(AbandonMatchIncidentType.RETIREMENT,
                            (teamId == TeamId.A ? TeamId.B : TeamId.A));
        }

        if (optionName.contains("WALKOVER")) {
            return new AbandonMatchIncident(AbandonMatchIncidentType.WALKOVER,
                            (teamId == TeamId.A ? TeamId.B : TeamId.A));
        }

        if (optionName.contains("BET_")) {
            return new DatafeedMatchIncident((DatafeedMatchIncidentType) incidentType, guiResponse.getElapsedTime());
        }

        if (optionName.contains("SCOUT_")) {
            return new AbandonMatchIncident(AbandonMatchIncidentType.SCOUT_ABANDONED, TeamId.UNKNOWN);
        }
        if (optionName.contains("CANCEL")) {
            AbandonMatchIncident abandonMatchIncident = new AbandonMatchIncident();
            abandonMatchIncident.setIncidentSubType(AbandonMatchIncidentType.CANCELLATION);
        ...
        if (optionName.contains("UNKNOWN")) {
