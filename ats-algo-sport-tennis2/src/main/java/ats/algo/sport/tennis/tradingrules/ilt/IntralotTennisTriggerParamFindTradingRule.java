...
public class IntralotTennisTriggerParamFindTradingRule extends TriggerParamFindTradingRule {

    private static final long serialVersionUID = 1L;


    protected IntralotTennisTriggerParamFindTradingRule() {
        super("Tennis TriggerParamFindingTradingRule", null);
    }
    ...
    protected String processPricesInPlay(MarketPricesList marketPricesList, AlgoMatchState matchState,
        ...
        boolean startOfAGame = currentPointSequenceId.endsWith(".1");
        ...
        if (!firstPoint)
            checkValidityOfNextGameMarkets(marketPricesList, currentGame, startOfAGame);
    ...
    protected void checkValidityOfNextGameMarkets(MarketPricesList marketPricesList, int currentGame,
        ...
        if (count < 1 && startOfAGame) {
            for (MarketPrices marketPrices : marketPricesList.values()) {
                for (MarketPrice marketPrice : marketPrices) {
                    if (marketPrice.getType().equals("G:ML")) {
                        if (SequenceId.getSequenceIdComparator(marketPrice.getSequenceId()) == currentGame) {
                            marketPrice.setValid(true);
                        }
                    }
                }
    ...
    protected boolean isInDeuceSequence(String sequenceId) {
        String pointId = sequenceId.substring(sequenceId.length() - 1);
    ...
    protected static void checkLatestGameWinnerPricesForValidity(MarketPricesList marketPricesList) {
        int temp = 0;
        int max = 0;
        for (MarketPrices marketPrices : marketPricesList.values()) {
            for (MarketPrice marketPrice : marketPrices) {
                if (marketPrice.getType().equals("G:ML")) {
                    max = SequenceId.getSequenceIdComparator(marketPrice.getSequenceId());
                    if (max > temp)
                        temp = max;
                }
            }
        ...
        for (MarketPrices marketPrices : marketPricesList.values()) {
            for (MarketPrice marketPrice : marketPrices) {
                if (marketPrice.getType().equals("G:ML")) {
                    if (max > SequenceId.getSequenceIdComparator(marketPrice.getSequenceId()))
                        marketPrice.setValid(false);
                }
            }
        }
