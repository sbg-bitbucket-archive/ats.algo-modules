...
public class BsTennisTradingRule extends SetSuspensionStatusTradingRule {
    ...
    private List<String> finalSetTiebreakSuspendList;
    ...
    private Map<String, List<String>> inplaySequenceIdFiltersMapUncertainSet;
    ...
    private List<String> breakPointSuspendMarketListDouble;
    private List<String> tieBreakSuspendMarketListDouble;

    // private List<String> injuryModeActiveSuspendMarketListDouble;
    // JSON
    public BsTennisTradingRule() {}
    ...
    public List<String> getBreakPointSuspendMarketListDouble() {
        return breakPointSuspendMarketListDouble;
    }

    public void setBreakPointSuspendMarketListDouble(List<String> tieBreakSuspendMarketListDouble) {
        this.breakPointSuspendMarketListDouble = tieBreakSuspendMarketListDouble;
    }

    public List<String> getTieBreakSuspendMarketListDouble() {
        return tieBreakSuspendMarketListDouble;
    }

    public void setTieBreakSuspendMarketListDouble(List<String> tieBreakSuspendMarketListDouble) {
        this.tieBreakSuspendMarketListDouble = tieBreakSuspendMarketListDouble;
    }

    public List<String> getFinalSetTiebreakSuspendList() {
        return finalSetTiebreakSuspendList;
    }

    public void setFinalSetTiebreakSuspendList(List<String> finalSetTiebreakSuspendList) {
        this.finalSetTiebreakSuspendList = finalSetTiebreakSuspendList;
    }

    public Map<String, List<String>> setInplaySequenceIdFilterListUncertainSet() {
        return inplaySequenceIdFiltersMapUncertainSet;
    }

    public void setInplaySequenceIdFilterListUncertainSet(
                    Map<String, List<String>> inplaySequenceIdFiltersMapUncertainSet) {
        this.inplaySequenceIdFiltersMapUncertainSet = inplaySequenceIdFiltersMapUncertainSet;
    }
     ...
     * @param marketTier
     */
    public BsTennisTradingRule(String ruleName, Integer eventTier, MarketGroup marketGroup) {
    ...
    public void applyRule(AlgoMatchState matchState, Market market, CalcRequestCause priceCalcCause,
        ...
        if (applyPreMatchRules) {
            ...
            if (inplayDisplayMarketList != null && !inList(marketType, inplayDisplayMarketList)) {
                ...
                if (isLastGameInMatch(tennisMatchState, tennisMatchFormat)) {
                    if (marketType.equals("G:ML")) {
                        marketStatus.setStatusIfHigherPriority(getRuleName(), SuspensionStatus.SUSPENDED_UNDISPLAY,
                                        "Suspend due to potenial last game in the match");
                    }
                }
                ...
                /*
                 * check final set tiebreak rule applies
                 */

                if (tennisMatchState.isInFinalSet() && tennisMatchState.isInTieBreak()
                                && inList(marketType, finalSetTiebreakSuspendList)) {
                    marketStatus.setStatusIfHigherPriority(getRuleName(), SuspensionStatus.SUSPENDED_UNDISPLAY,
                                    "Suspend during final set tie break");
                }
                ...
                int numSetsNeededToWin = tennisMatchState.getNoSetsInMatch() / 2 + 1;
                if (tennisMatchState.getNoSetsInMatch() == 5) {
                    if (tennisMatchState.getSetNo() > 2 && (((tennisMatchState.getSetsA() + 1) == numSetsNeededToWin)
                                    || ((tennisMatchState.getSetsB() + 1) == numSetsNeededToWin))) {
                        if (filterOutThroughSequenceId(market.getType(), mktSequenceId,
                                        matchState.getSequenceId(mktSequenceId, 0),
                                        this.inplaySequenceIdFiltersMapUncertainSet)) {
                            marketStatus.setStatusIfHigherPriority(getRuleName(), SuspensionStatus.SUSPENDED_UNDISPLAY,
                                            "Suspend potential next set markets when one player needs only this set to win");
                        }
                    }
                } else {
                    if (tennisMatchState.getSetNo() > 1 && (((tennisMatchState.getSetsA() + 1) == numSetsNeededToWin)
                                    || ((tennisMatchState.getSetsB() + 1) == numSetsNeededToWin))) {
                        if (filterOutThroughSequenceId(market.getType(), mktSequenceId,
                                        matchState.getSequenceId(mktSequenceId, 0),
                                        this.inplaySequenceIdFiltersMapUncertainSet)) {
        ...
        if (tennisMatchState.isDoubleMatch()) {
            ...
            if (tennisMatchState.getDataFeedStatus() == DatafeedMatchIncidentType.BET_STOP) {// if injury mode active,
                                                                                             // from tennis match
                // state see only data feed status triggers bet_stop
                // Markets suspension already implemented
                return;
            }



            if (tennisMatchState.isBreakPoint() && inList(marketType, breakPointSuspendMarketListDouble)) {
                marketStatus.setStatusIfHigherPriority(getRuleName(), SuspensionStatus.SUSPENDED_DISPLAY,
                                "Suspend on break point doubles");
            }

            if (tennisMatchState.isInTieBreak() && inList(marketType, tieBreakSuspendMarketListDouble)) {
                marketStatus.setStatusIfHigherPriority(getRuleName(), SuspensionStatus.SUSPENDED_UNDISPLAY,
                                "Suspend on tie break doubles");
            }

            if ((tennisMatchState.getPointsB() > 2 || tennisMatchState.getPointsA() > 2)
                            && inList(marketType, threePointSuspendMarketList)) {
                marketStatus.setStatusIfHigherPriority(getRuleName(), SuspensionStatus.SUSPENDED_UNDISPLAY,
                                "Suspend after 3 points for doubles");
            }
    ...
    private boolean isLastGameInMatch(TennisMatchState tennisMatchState, TennisMatchFormat tennisMatchFormat) {
        int set = (tennisMatchFormat.getSetsPerMatch() + 1) / 2;
        int currentSetA = tennisMatchState.getSetsA();
        int currentSetB = tennisMatchState.getSetsB();
        int currentGameA = tennisMatchState.getGamesA();
        int currentGameB = tennisMatchState.getGamesB();
        if (currentSetA == set - 1 && currentGameA > currentGameB && currentGameA >= 5)
            return true;
        if (currentSetB == set - 1 && currentGameB > currentGameA && currentGameB >= 5)
            return true;
        return false;
    }
