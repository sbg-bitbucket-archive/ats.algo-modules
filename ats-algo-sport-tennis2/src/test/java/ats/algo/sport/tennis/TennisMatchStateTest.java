...
public class TennisMatchStateTest {
    ...
    public void testAceCounter() {
        ...
        TennisMatchIncident incident = new TennisMatchIncident(0, TennisMatchIncidentType.MATCH_STARTING, TeamId.A, 0);
        matchState.updateStateForIncident(incident, false);
        incident = null;

        incident = new TennisMatchIncident(0, TennisMatchIncidentType.POINT_WON, TeamId.A, 0);
        matchState.updateStateForIncident(incident, false);
        incident = null;

        incident = new TennisMatchIncident(0, TennisMatchIncidentType.POINT_WON, TeamId.A, 0);
        incident.setPointResult(TennisPointResult.ACE);
        matchState.updateStateForIncident(incident, false);
        incident = null;
        assertEquals(1, matchState.getTotalAcesA());
        assertEquals(0, matchState.getTotalAcesB());

        incident = new TennisMatchIncident(0, TennisMatchIncidentType.POINT_WON, TeamId.A, 0);
        incident.setPointResult(TennisPointResult.ACE);
        matchState.updateStateForIncident(incident, false);
        incident = null;
        assertEquals(2, matchState.getTotalAcesA());
        assertEquals(0, matchState.getTotalAcesB());
        ...
        incident = new TennisMatchIncident(0, TennisMatchIncidentType.POINT_WON, TeamId.B, 0);
        incident.setPointResult(TennisPointResult.ACE);
        matchState.updateStateForIncident(incident, false);
        incident = null;
        assertEquals(2, matchState.getTotalAcesA());
        assertEquals(1, matchState.getTotalAcesB());
        ...
        incident = null;
        assertEquals(2, matchState.getTotalAcesA());
        assertEquals(2, matchState.getTotalAcesB());
        ...
        assertEquals(6, matchState.getTotalAcesA());
        assertEquals(2, matchState.getTotalAcesB());
        for (int i = 0; i < matchState.getSetNo(); i++) {
            // System.out.println(matchState.getAcesInSetN(i));

        }
        assertEquals(matchState.getAcesInSetN(0).A, 2);
        assertEquals(matchState.getAcesInSetN(0).B, 2);
        assertEquals(matchState.getAcesInSetN(1).A, 4);
        assertEquals(matchState.getAcesInSetN(1).B, 0);

        TennisSimpleMatchState tennisSimpleMatchState = matchState.generateSimpleMatchState();

        TennisMatchStateFromFeed tennisMatchStateFromFeed = tennisSimpleMatchState.generateMatchStateFromFeed();

        // System.out.println(tennisSimpleMatchState);
        // System.out.println(tennisMatchStateFromFeed);

        assertEquals(matchState.getPointsA(), tennisMatchStateFromFeed.getPointsA());
        assertEquals(matchState.getPointsB(), tennisMatchStateFromFeed.getPointsB());
        assertEquals(matchState.getGamesA(), tennisMatchStateFromFeed.getGamesA());
        assertEquals(matchState.getGamesA(), tennisMatchStateFromFeed.getGamesA());
        assertEquals(matchState.getSetsA(), tennisMatchStateFromFeed.getSetsA());
        assertEquals(matchState.getSetsB(), tennisMatchStateFromFeed.getSetsB());
        assertEquals(matchState.isInTieBreak(), tennisMatchStateFromFeed.isTiebreak());
        assertEquals(matchState.getOnServeNow(), tennisMatchStateFromFeed.getTeamServingNow());
    ...
    // **/
    // @Test //
    // public void testDoubleMixedGendersHandling() {
    // MethodName.print();
    // TennisMatchFormat matchFormat = new TennisMatchFormat(true, Sex.MIXED, Surface.HARD, TournamentLevel.ATP, 3, 7,
    // FinalSetType.CHAMPIONSHIP_TIE_BREAK, true, Sex.MEN, Sex.WOMEN);
    // matchState = new TennisMatchState(matchFormat);
    // assertTrue(matchState.isPreMatch());
    // matchState.setScore(0, 0, 0, 0, TeamId.UNKNOWN, 0, 0);
    // // System.out.println("Start of the match serve team/player - " + matchState.getStartOfMatchServer());
    //
    // // System.out.println(matchState.getGenderMap().get(PlayerId.B2));
    // assertEquals(matchState.getGenderMap().get(PlayerId.B2), GenderId.MALE);
    // }
    ...
    public void setIncidentTeamIdTest() {
        ...
        // Null test
        TennisMatchIncident matchIncident = null;
        matchState.setIncidentTeamId(matchIncident);
        ...
        matchIncident = new TennisMatchIncident(2, TennisMatchIncidentType.POINT_WON, TeamId.B, 2);
        matchState.setIncidentTeamId(matchIncident);
        assertEquals(matchState.getIncidentTeamId(), TeamId.B.toString());

        matchIncident = new TennisMatchIncident(3, TennisMatchIncidentType.FAULT, TeamId.A, 1);
        matchState.setOnServeSideNow(TeamId.A);
        matchState.setIncidentTeamId(matchIncident);
        assertEquals(matchState.getIncidentTeamId(), TeamId.A.toString());

        // Even if the match incident has Team.B, ServerNow player is added to match incident on FAULT
        matchIncident = new TennisMatchIncident(4, TennisMatchIncidentType.FAULT, TeamId.B, 2);
        matchState.setIncidentTeamId(matchIncident);
        assertEquals(matchState.getIncidentTeamId(), TeamId.A.toString());
    }
}
