...
public class PlayDart {
    ...
    private class CheckoutEntry {
        ...
        /// <param name="nDartsToFinish">min no of darts required to finish from current score</param>
        /// <param name="fewerDartsViaBull">true if can finish in fewer darts by hitting inner bull</param>
        /// <param name="nextNoStd">next no to aim at in format mnn, where m =2,3 is multiplier;</param>
        /// <param name="nextNoBull">next no to aim at in format mnn, where m =2,3 is multiplier;</param>
        public CheckoutEntry(int score, int nDartsToFinish, boolean fewerDartsViaBull, int nextNoStd, int nextNoBull) {
            ...
            this.nextNoIfStd = dartTarget.no;
            this.multiplierIfStd = dartTarget.multiplier;
            dartTarget = setTarget(nextNoBull);
            this.nextNoIfViaBull = dartTarget.no;
            this.multiplierIfBull = dartTarget.multiplier;
        }
    }

    /// <summary>
    /// contains the data required to determine the next dart target for any score up to 170
    /// </summary>
    ...
    public DartTarget throwDart(DartProbs playerAtOche, LegState score, DartTarget target) {
        ...
        if (score.doubleReqdToStart && score.scoreAtOche() == 501) {
            ...
            if (score.scoreAtOche() > 170) {
                boolean twoTripleTwentiesAlready = (score.threeDartSet.noDartsThrown == 2)
                                && score.threeDartSet.hitT20(1) && score.threeDartSet.hitT20(2);
                boolean aimforT20 = true;
                target.multiplier = 3; // triple
                if (twoTripleTwentiesAlready)
                    aimforT20 = !aimForT19();
                if (aimforT20)
                    target.no = adjustTargetToAvoidDeadNumbers(score.scoreAtOche(), 20);
                else
                    target.no = adjustTargetToAvoidDeadNumbers(score.scoreAtOche(), 19);
            } else {
                ...
                if (restPlayerCanCheckoutInTwoDarts(score.scoreAtRest()) && checkoutEntry.fewerDartsViaBull) {
                    // go for bull
                    target.multiplier = checkoutEntry.multiplierIfBull;
                    target.no = checkoutEntry.nextNoIfViaBull;
                } else {
                    // standard checkout
                    target.multiplier = checkoutEntry.multiplierIfStd;
                    target.no = checkoutEntry.nextNoIfStd;
                }
            }
        }
        //
    ...
    private DartTarget executeThrow(DartProbs probs, DartTarget target) {
        ...
        if (target.no == 25)
            ...
            aimedAt = target.multiplier;
        DartTarget actual = probs.getActual(aimedAt, target.no);
        return actual;
    }
}
