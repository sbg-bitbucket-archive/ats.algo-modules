...
public class FootballTradingRule extends SetSuspensionStatusTradingRule {
    ...
    private double minSelectionProb = 0.0;
    private double maxSelectionProb = 0.0;
    ...
    public int getElapsedMinutesForSuspensionOfMarkets() {
        return elapsedMinutesForSuspensionOfMarkets;
    }

    public void setElapsedMinutesForSuspensionOfMarkets(int elapsedMinutesForSuspensionOfMarkets) {
        this.elapsedMinutesForSuspensionOfMarkets = elapsedMinutesForSuspensionOfMarkets;
    }
    ...
    public void setParamFindSuspensionList(Map<String, MarketGroup> paramFindSuspensionList) {
        this.paramFindSuspensionList = paramFindSuspensionList;
    }

    public double getMinSelectionProb() {
        return minSelectionProb;
    }

    public void setMinSelectionProb(double minSelectionProb) {
        this.minSelectionProb = minSelectionProb;
    }

    public double getMaxSelectionProb() {
        return maxSelectionProb;
    }

    public void setMaxSelectionProb(double maxSelectionProb) {
        this.maxSelectionProb = maxSelectionProb;
    }
    ...
    public void applyRule(AlgoMatchState matchState, Market market, CalcRequestCause priceCalcCause,
        ...
        if (maxSelectionProb != 0.0 && market.getSelections().size() == 2) {
            for (Selection selection : market.getSelections().values()) {
                if (selection.getProb() > maxSelectionProb) {
                    marketStatus.setStatusIfHigherPriority(getRuleName(), SuspensionStatus.SUSPENDED_UNDISPLAY,
                                    "Suspending market as outside of prob boundary (" + maxSelectionProb + ")");
                }
            }
        }

        if (minSelectionProb != 0.0 && market.getSelections().size() == 2) {
            for (Selection selection : market.getSelections().values()) {
                if (selection.getProb() < minSelectionProb) {
                    marketStatus.setStatusIfHigherPriority(getRuleName(), SuspensionStatus.SUSPENDED_UNDISPLAY,
                                    "Suspending market as outside of prob boundary (" + minSelectionProb + ")");
                }
            }
        }
                            ...
                            if (priceCalcCause != null)
                                if (market.getMarketGroup().equals(MarketGroup.CORNERS)
                                    ...
                                    marketStatus.setStatusIfHigherPriority(getRuleName(),
                                                    ...
                                                    "Suspend and display for a corner");
                            break;
                        case YELLOW_CARD:
