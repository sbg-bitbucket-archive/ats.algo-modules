...
public class FootballMatchMarketsFactory extends MarketsFactory {
    ...
    private static final boolean GENERATE_GS_MARKETS = false;
    ...
    private NWayStatistic buildABet;
    ...
    private NWayStatistic correctScoreDS;
    private NWayStatistic correctScoreDSHalf;
    ...
    private static final String[] correctScoreDSHalfList =
                    {"1-0", "0-1", "2-0", "0-2", "2-1", "1-2", "0-0", "1-1", "2-2", "Any Other"}; // FT:CSDS
    private static final String[] correctScoreDSList = {"1-0", "0-1", "2-0", "0-2", "2-1", "1-2", "0-0", "1-1", "2-2",
            "3-0", "0-3", "3-1", "1-3", "3-2", "2-3", "4-0", "0-4", "4-1", "1-4", "4-2", "2-4", "5-0", "0-5", "5-1",
            "1-5", "6-0", "0-6", "3-3", "Any Other"}; // P:CSDS
    ...
    private static double correctScoreLimit = 0.00001;
    private static boolean prematch;

    private static List<String> babSelection = new ArrayList<>();
    private final double babLimit = 0.00001;
    // <<<<< Half time scores generation variables
    ...
    FootballMatchMarketsFactory(FootballMatchState matchState, boolean isForParamFinder) {
        ...
        buildABet = new NWayStatistic("FT:BAB", "Build A Bet", MarketGroup.GOALS, isNormalTimeMatchFinished, "M",
                        babSelection.toArray(new String[babSelection.size()]));
        buildABet.setnWayLimit(babLimit);
        ...
        goalsTotalA = new TotalStatistic("FT:A:OU", "Total Goals Home Team", MarketGroup.GOALS,
                        isNormalTimeMatchFinished, "M", maxNoGoalsPerTeam);
        ...
        secondHalfHomeTeamToWinBTTS =
                        new TwoWayStatistic("FT:BTTS2HH", "Second Half - Home Team to Win & Both Teams to Score",
                                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");

        secondHalfAwayTeamToWinBTTS =
                        new TwoWayStatistic("FT:BTTS2HA", "Second Half - Away Team to Win & Both Teams to Score",
                                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");

        secondHalfDrawBTTS = new TwoWayStatistic("FT:BTTS2HX", "Second Half - Draw & Both Teams to Score",
                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");

        secondHalfHomeTeamToWinBTNTS =
                        new TwoWayStatistic("FT:BTNTS2HH", "Second Half - Home Team to Win & Both Teams Not to Score",
                                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");

        secondHalfAwayTeamToWinBTNTS =
                        new TwoWayStatistic("FT:BTNTS2HA", "Second Half - Away Team to Win & Both Teams Not to Score",
                                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");

        secondHalfDrawBTNTS = new TwoWayStatistic("FT:BTNTS2HX", "Second Half - Draw & Both Teams Not to Score",
                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");
        ...
        firstHalfNormalTimeScores.setnWayLimit(babLimit);
        ...
        firstHalfHomeTeamToWinBTTS =
                        new TwoWayStatistic("FT:BTTS1HH", "First Half - Home Team to Win & Both Teams to Score",
                                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");

        firstHalfAwayTeamToWinBTTS =
                        new TwoWayStatistic("FT:BTTS1HA", "First Half - Away Team to Win & Both Teams to Score",
                                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");

        firstHalfDrawBTTS = new TwoWayStatistic("FT:BTTS1HX", "First Half - Draw & Both Teams to Score",
                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");

        firstHalfHomeTeamToWinBTNTS =
                        new TwoWayStatistic("FT:BTNTS1HH", "First Half - Home Team to Win & Both Teams Not to Score",
                                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");

        firstHalfAwayTeamToWinBTNTS =
                        new TwoWayStatistic("FT:BTNTS1HA", "First Half - Away Team to Win & Both Teams Not to Score",
                                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");

        firstHalfDrawBTNTS = new TwoWayStatistic("FT:BTNTS1HX", "First Half - Draw & Both Teams Not to Score",
                        MarketGroup.GOALS, isNormalTimeMatchFinished, "M", "Yes", "No");
        ...
        firstHWinBndScore23 = new TwoWayStatistic("P:B:TW23", string7, MarketGroup.GOALS, periodNo < 3,
                        periodSequenceId, "Yes", "No");
        generateMarket = true;
        correctScoreDS = new NWayStatistic("FT:CSDS", "Correct score DS ", MarketGroup.GOALS, generateMarket, "M",
                        correctScoreDSList);

        String csDS = "1st Half Correct Score DS";
        if (periodNo < 2) {
            correctScoreDSHalf = new NWayStatistic("P:CSDS", csDS, MarketGroup.GOALS, generateMarket, "P1",
                            correctScoreDSHalfList);
        } else if (periodNo <= 3) {
            csDS = "2nd Half Correct Score DS";
            correctScoreDSHalf = new NWayStatistic("P:CSDS", csDS, MarketGroup.GOALS, generateMarket, "P2",
                            correctScoreDSHalfList);
        }
    ...
    void updateStats(FootballMatchState matchState, FootballMatchFacts matchFacts) {
        ...
        String csDS = normalTimeGoalsA + "-" + normalTimeGoalsB;
        int cs = 0;
        while (!correctScoreDSList[cs].equals(csDS)) {
            if (cs == correctScoreDSList.length - 1)
                break;
            else
                cs++;
        }
        correctScoreDS.increment(cs);
        cs = 0;
        String csDSHalf = goalCurrentHalfA + "-" + goalCurrentHalfB;
        while (!correctScoreDSHalfList[cs].equals(csDSHalf)) {
            if (cs == correctScoreDSHalfList.length - 1)
                break;
            else
                cs++;
        }

        correctScoreDSHalf.increment(cs);
        ...
        Integer firstHalfNormalTimeScoreIndex =
                        getFirstHalfNormalTimeScoreSelectionIndex(goalsAfirstHalf, goalsBfirstHalf, goalsA, goalsB);
        ...
        if (matchState.didTeamScoreFirst(TeamId.A))
            babWinSelection += "A,H:";
        else if (matchState.didTeamScoreFirst(TeamId.B))
            babWinSelection += "B,H:";
        else
            babWinSelection += "X,H:";
        String halfFullScore = goalsAfirstHalf + "-" + goalsBfirstHalf + ",C:" + goalsA + "-" + goalsB;
        babWinSelection += halfFullScore;

        int babIndex = babSelection.indexOf(babWinSelection);
        if (babIndex != -1)
            buildABet.increment(babIndex);
        // Corner market
    ...
    public void addDerivedMarkets(Markets markets, AlgoMatchState matchState, MatchParams matchParams) {
        ...
        if (!((FootballMatchState) matchState).isNormalTimeMatchCompleted()) {
            ...
            // cs
            market = new Market(MarketCategory.GENERAL, MarketGroup.GOALS, "FT:£CS_2", "M",
            ...
            if (((FootballMatchState) matchState).getPeriodNo() < 2) {
                probs = generateHFPOUProbs(scoreGridPeriod, scoreGrid, 2.5);
    ...
    private void initBabSelection() {
        /*
         * Arrays.asList("F:A,H:0-0,C:1-0", "F:A,H:0-0,C:1-1", "F:A,H:0-0,C:1-2", "F:A,H:0-0,C:1-3", "F:A,H:0-0,C:1-4",
         * "F:A,H:0-0,C:2-0", "F:A,H:0-0,C:2-1", "F:A,H:0-0,C:2-2", "F:A,H:0-0,C:2-3", "F:A,H:0-0,C:2-4",
         * "F:A,H:0-0,C:3-0", "F:A,H:0-0,C:3-1", "F:A,H:0-0,C:3-2", "F:A,H:0-0,C:3-3", "F:A,H:0-0,C:4-0",
         * "F:A,H:0-0,C:4-1", "F:A,H:0-0,C:4-2", "F:A,H:0-0,C:4-3", "F:A,H:0-0,C:5-0", "F:A,H:0-0,C:5-1",
         * "F:A,H:0-0,C:5-2", "F:A,H:0-0,C:6-0", "F:A,H:0-0,C:6-1", "F:A,H:0-0,C:7-0", "F:A,H:1-0,C:1-0",
         * "F:A,H:1-0,C:1-1", "F:A,H:1-0,C:1-2", "F:A,H:1-0,C:1-3", "F:A,H:1-0,C:1-4", "F:A,H:1-0,C:1-5",
         * "F:A,H:1-0,C:2-0", "F:A,H:1-0,C:2-1", "F:A,H:1-0,C:2-2", "F:A,H:1-0,C:2-3", "F:A,H:1-0,C:2-4",
         * "F:A,H:1-0,C:2-5", "F:A,H:1-0,C:3-0", "F:A,H:1-0,C:3-1", "F:A,H:1-0,C:3-2", "F:A,H:1-0,C:3-3",
         * "F:A,H:1-0,C:3-4", "F:A,H:1-0,C:4-0", "F:A,H:1-0,C:4-1", "F:A,H:1-0,C:4-2", "F:A,H:1-0,C:4-3",
         * "F:A,H:1-0,C:4-4", "F:A,H:1-0,C:5-0", "F:A,H:1-0,C:5-1", "F:A,H:1-0,C:5-2", "F:A,H:1-0,C:5-3",
         * "F:A,H:1-0,C:6-0", "F:A,H:1-0,C:6-1", "F:A,H:1-0,C:6-2", "F:A,H:1-0,C:6-3", "F:A,H:1-0,C:7-0",
         * "F:A,H:1-0,C:7-1", "F:A,H:1-0,C:7-2", "F:A,H:1-0,C:8-0", "F:A,H:1-1,C:1-1", "F:A,H:1-1,C:1-2",
         * "F:A,H:1-1,C:1-3", "F:A,H:1-1,C:1-4", "F:A,H:1-1,C:1-5", "F:A,H:1-1,C:2-1", "F:A,H:1-1,C:2-2",
         * "F:A,H:1-1,C:2-3", "F:A,H:1-1,C:2-4", "F:A,H:1-1,C:2-5", "F:A,H:1-1,C:2-6", "F:A,H:1-1,C:3-1",
         * "F:A,H:1-1,C:3-2", "F:A,H:1-1,C:3-3", "F:A,H:1-1,C:3-4", "F:A,H:1-1,C:3-5", "F:A,H:1-1,C:4-1",
         * "F:A,H:1-1,C:4-2", "F:A,H:1-1,C:4-3", "F:A,H:1-1,C:4-4", "F:A,H:1-1,C:5-1", "F:A,H:1-1,C:5-2",
         * "F:A,H:1-1,C:5-3", "F:A,H:1-1,C:6-1", "F:A,H:1-1,C:6-2", "F:A,H:1-1,C:7-1", "F:A,H:1-2,C:1-2",
         * "F:A,H:1-2,C:1-3", "F:A,H:1-2,C:1-4", "F:A,H:1-2,C:1-5", "F:A,H:1-2,C:2-2", "F:A,H:1-2,C:2-3",
         * "F:A,H:1-2,C:2-4", "F:A,H:1-2,C:2-5", "F:A,H:1-2,C:3-2", "F:A,H:1-2,C:3-3", "F:A,H:1-2,C:3-4",
         * "F:A,H:1-2,C:4-2", "F:A,H:1-2,C:4-3", "F:A,H:1-2,C:4-4", "F:A,H:1-2,C:5-2", "F:A,H:1-2,C:5-3",
         * "F:A,H:1-2,C:6-2", "F:A,H:1-3,C:1-3", "F:A,H:1-3,C:1-4", "F:A,H:1-3,C:2-3", "F:A,H:1-3,C:2-4",
         * "F:A,H:1-3,C:2-5", "F:A,H:1-3,C:3-3", "F:A,H:1-3,C:3-4", "F:A,H:1-3,C:4-3", "F:A,H:2-0,C:2-0",
         * "F:A,H:2-0,C:2-1", "F:A,H:2-0,C:2-2", "F:A,H:2-0,C:2-3", "F:A,H:2-0,C:2-4", "F:A,H:2-0,C:3-0",
         * "F:A,H:2-0,C:3-1", "F:A,H:2-0,C:3-2", "F:A,H:2-0,C:3-3", "F:A,H:2-0,C:3-4", "F:A,H:2-0,C:4-0",
         * "F:A,H:2-0,C:4-1", "F:A,H:2-0,C:4-2", "F:A,H:2-0,C:4-3", "F:A,H:2-0,C:4-4", "F:A,H:2-0,C:5-0",
         * "F:A,H:2-0,C:5-1", "F:A,H:2-0,C:5-2", "F:A,H:2-0,C:5-3", "F:A,H:2-0,C:6-0", "F:A,H:2-0,C:6-1",
         * "F:A,H:2-0,C:6-2", "F:A,H:2-0,C:7-0", "F:A,H:2-0,C:7-1", "F:A,H:2-0,C:7-2", "F:A,H:2-0,C:8-0",
         * "F:A,H:2-0,C:8-1", "F:A,H:2-1,C:2-1", "F:A,H:2-1,C:2-2", "F:A,H:2-1,C:2-3", "F:A,H:2-1,C:2-4",
         * "F:A,H:2-1,C:2-5", "F:A,H:2-1,C:3-1", "F:A,H:2-1,C:3-2", "F:A,H:2-1,C:3-3", "F:A,H:2-1,C:3-4",
         * "F:A,H:2-1,C:4-1", "F:A,H:2-1,C:4-2", "F:A,H:2-1,C:4-3", "F:A,H:2-1,C:4-4", "F:A,H:2-1,C:5-1",
         * "F:A,H:2-1,C:5-2", "F:A,H:2-1,C:5-3", "F:A,H:2-1,C:5-4", "F:A,H:2-1,C:6-1", "F:A,H:2-1,C:6-2",
         * "F:A,H:2-1,C:6-3", "F:A,H:2-1,C:7-1", "F:A,H:2-1,C:7-2", "F:A,H:2-1,C:8-1", "F:A,H:2-2,C:2-2",
         * "F:A,H:2-2,C:2-3", "F:A,H:2-2,C:2-4", "F:A,H:2-2,C:2-6", "F:A,H:2-2,C:3-2", "F:A,H:2-2,C:3-3",
         * "F:A,H:2-2,C:3-4", "F:A,H:2-2,C:3-5", "F:A,H:2-2,C:4-2", "F:A,H:2-2,C:4-3", "F:A,H:2-2,C:4-4",
         * "F:A,H:2-2,C:5-2", "F:A,H:2-2,C:5-3", "F:A,H:2-2,C:5-4", "F:A,H:2-2,C:6-2", "F:A,H:2-2,C:6-3",
         * "F:A,H:2-3,C:2-3", "F:A,H:2-3,C:2-4", "F:A,H:2-3,C:3-3", "F:A,H:2-3,C:3-4", "F:A,H:2-3,C:4-3",
         * "F:A,H:2-3,C:5-3", "F:A,H:2-3,C:4-4", "F:A,H:3-0,C:3-0", "F:A,H:3-0,C:3-1", "F:A,H:3-0,C:3-2",
         * "F:A,H:3-0,C:3-3", "F:A,H:3-0,C:3-4", "F:A,H:3-0,C:4-0", "F:A,H:3-0,C:4-1", "F:A,H:3-0,C:4-2",
         * "F:A,H:3-0,C:4-3", "F:A,H:3-0,C:5-0", "F:A,H:3-0,C:5-1", "F:A,H:3-0,C:5-2", "F:A,H:3-0,C:5-3",
         * "F:A,H:3-0,C:6-0", "F:A,H:3-0,C:6-1", "F:A,H:3-0,C:6-2", "F:A,H:3-0,C:6-3", "F:A,H:3-0,C:7-0",
         * "F:A,H:3-0,C:7-1", "F:A,H:3-0,C:7-2", "F:A,H:3-0,C:8-0", "F:A,H:3-0,C:8-1", "F:A,H:3-0,C:9-0",
         * "F:A,H:3-1,C:3-1", "F:A,H:3-1,C:3-2", "F:A,H:3-1,C:3-3", "F:A,H:3-1,C:3-4", "F:A,H:3-1,C:4-1",
         * "F:A,H:3-1,C:4-2", "F:A,H:3-1,C:4-3", "F:A,H:3-1,C:4-4", "F:A,H:3-1,C:5-1", "F:A,H:3-1,C:5-2",
         * "F:A,H:3-1,C:5-3", "F:A,H:3-1,C:5-4", "F:A,H:3-1,C:6-1", "F:A,H:3-1,C:6-2", "F:A,H:3-1,C:6-3",
         * "F:A,H:3-1,C:7-1", "F:A,H:3-1,C:7-2", "F:A,H:3-1,C:8-1", "F:A,H:3-2,C:3-2", "F:A,H:3-2,C:3-3",
         * "F:A,H:3-2,C:3-4", "F:A,H:3-2,C:4-2", "F:A,H:3-2,C:4-3", "F:A,H:3-2,C:4-4", "F:A,H:3-2,C:5-2",
         * "F:A,H:3-2,C:5-3", "F:A,H:3-2,C:5-4", "F:A,H:3-2,C:6-2", "F:A,H:3-2,C:6-3", "F:A,H:3-3,C:3-3",
         * "F:A,H:3-3,C:3-4", "F:A,H:3-3,C:4-3", "F:A,H:3-4,C:4-7", "F:A,H:4-0,C:4-0", "F:A,H:4-0,C:4-1",
         * "F:A,H:4-0,C:4-2", "F:A,H:4-0,C:4-3", "F:A,H:4-0,C:5-0", "F:A,H:4-0,C:5-1", "F:A,H:4-0,C:5-2",
         * "F:A,H:4-0,C:5-3", "F:A,H:4-0,C:6-0", "F:A,H:4-0,C:6-1", "F:A,H:4-0,C:6-2", "F:A,H:4-0,C:6-3",
         * "F:A,H:4-0,C:7-0", "F:A,H:4-0,C:7-1", "F:A,H:4-0,C:7-2", "F:A,H:4-0,C:8-0", "F:A,H:4-0,C:8-1",
         * "F:A,H:4-0,C:9-0", "F:A,H:4-1,C:4-1", "F:A,H:4-1,C:4-2", "F:A,H:4-1,C:4-3", "F:A,H:4-1,C:4-4",
         * "F:A,H:4-1,C:5-1", "F:A,H:4-1,C:5-2", "F:A,H:4-1,C:5-3", "F:A,H:4-1,C:6-1", "F:A,H:4-1,C:6-2",
         * "F:A,H:4-1,C:6-3", "F:A,H:4-1,C:7-1", "F:A,H:4-1,C:7-2", "F:A,H:4-1,C:8-1", "F:A,H:4-2,C:4-2",
         * "F:A,H:4-2,C:4-3", "F:A,H:4-2,C:5-2", "F:A,H:4-2,C:5-3", "F:A,H:4-2,C:5-4", "F:A,H:4-2,C:6-2",
         * "F:A,H:5-0,C:5-0", "F:A,H:5-0,C:5-1", "F:A,H:5-0,C:5-2", "F:A,H:5-0,C:6-0", "F:A,H:5-0,C:6-1",
         * "F:A,H:5-0,C:6-2", "F:A,H:5-0,C:7-0", "F:A,H:5-0,C:7-1", "F:A,H:5-0,C:7-2", "F:A,H:5-0,C:8-0",
         * "F:A,H:5-0,C:8-1", "F:A,H:5-1,C:5-1", "F:A,H:5-1,C:5-2", "F:A,H:5-1,C:6-1", "F:A,H:5-1,C:6-2",
         * "F:A,H:5-1,C:6-3", "F:A,H:5-1,C:7-1", "F:A,H:5-1,C:7-2", "F:A,H:6-0,C:6-0", "F:A,H:6-0,C:6-1",
         * "F:A,H:6-0,C:7-0", "F:A,H:6-0,C:7-1", "F:A,H:6-0,C:8-0", "F:A,H:6-1,C:7-1", "F:B,H:0-0,C:0-1",
         * "F:B,H:0-0,C:0-2", "F:B,H:0-0,C:0-3", "F:B,H:0-0,C:0-4", "F:B,H:0-0,C:1-1", "F:B,H:0-0,C:1-2",
         * "F:B,H:0-0,C:1-3", "F:B,H:0-0,C:1-4", "F:B,H:0-0,C:2-1", "F:B,H:0-0,C:2-2", "F:B,H:0-0,C:2-3",
         * "F:B,H:0-0,C:2-4", "F:B,H:0-0,C:3-1", "F:B,H:0-0,C:3-2", "F:B,H:0-0,C:3-3", "F:B,H:0-0,C:4-1",
         * "F:B,H:0-0,C:4-2", "F:B,H:0-0,C:4-3", "F:B,H:0-0,C:5-1", "F:B,H:0-0,C:5-2", "F:B,H:0-1,C:0-1",
         * "F:B,H:0-1,C:0-2", "F:B,H:0-1,C:0-3", "F:B,H:0-1,C:0-4", "F:B,H:0-1,C:0-5", "F:B,H:0-1,C:1-1",
         * "F:B,H:0-1,C:1-2", "F:B,H:0-1,C:1-3", "F:B,H:0-1,C:1-4", "F:B,H:0-1,C:1-5", "F:B,H:0-1,C:2-1",
         * "F:B,H:0-1,C:2-2", "F:B,H:0-1,C:2-3", "F:B,H:0-1,C:2-4", "F:B,H:0-1,C:2-5", "F:B,H:0-1,C:3-1",
         * "F:B,H:0-1,C:3-2", "F:B,H:0-1,C:3-3", "F:B,H:0-1,C:3-4", "F:B,H:0-1,C:4-1", "F:B,H:0-1,C:4-2",
         * "F:B,H:0-1,C:4-3", "F:B,H:0-1,C:4-4", "F:B,H:0-1,C:5-1", "F:B,H:0-1,C:5-2", "F:B,H:0-1,C:6-1",
         * "F:B,H:0-2,C:0-2", "F:B,H:0-2,C:0-3", "F:B,H:0-2,C:0-4", "F:B,H:0-2,C:0-5", "F:B,H:0-2,C:1-2",
         * "F:B,H:0-2,C:1-3", "F:B,H:0-2,C:1-4", "F:B,H:0-2,C:1-5", "F:B,H:0-2,C:2-2", "F:B,H:0-2,C:2-3",
         * "F:B,H:0-2,C:2-4", "F:B,H:0-2,C:2-5", "F:B,H:0-2,C:3-2", "F:B,H:0-2,C:3-3", "F:B,H:0-2,C:3-4",
         * "F:B,H:0-2,C:3-5", "F:B,H:0-2,C:4-2", "F:B,H:0-2,C:4-3", "F:B,H:0-2,C:4-4", "F:B,H:0-2,C:5-2",
         * "F:B,H:0-2,C:5-3", "F:B,H:0-3,C:0-3", "F:B,H:0-3,C:0-4", "F:B,H:0-3,C:0-5", "F:B,H:0-3,C:1-3",
         * "F:B,H:0-3,C:1-4", "F:B,H:0-3,C:1-5", "F:B,H:0-3,C:2-3", "F:B,H:0-3,C:2-4", "F:B,H:0-3,C:2-5",
         * "F:B,H:0-3,C:3-3", "F:B,H:0-3,C:3-4", "F:B,H:0-3,C:4-3", "F:B,H:0-3,C:4-4", "F:B,H:0-3,C:4-5",
         * "F:B,H:0-4,C:0-4", "F:B,H:0-4,C:0-5", "F:B,H:0-4,C:1-4", "F:B,H:0-4,C:1-5", "F:B,H:0-4,C:1-6",
         * "F:B,H:0-4,C:2-4", "F:B,H:0-4,C:3-7", "F:B,H:0-5,C:1-5", "F:B,H:1-1,C:1-1", "F:B,H:1-1,C:1-2",
         * "F:B,H:1-1,C:1-3", "F:B,H:1-1,C:1-4", "F:B,H:0-4,C:2-6", "F:B,H:1-1,C:1-5", "F:B,H:1-1,C:2-1",
         * "F:B,H:1-1,C:2-2", "F:B,H:1-1,C:2-3", "F:B,H:1-1,C:2-4", "F:B,H:1-1,C:2-5", "F:B,H:1-1,C:3-1",
         * "F:B,H:1-1,C:3-2", "F:B,H:1-1,C:3-3", "F:B,H:1-1,C:3-4", "F:B,H:1-1,C:3-5", "F:B,H:1-1,C:4-1",
         * "F:B,H:1-1,C:4-2", "F:B,H:1-1,C:4-3", "F:B,H:1-1,C:4-4", "F:B,H:1-1,C:5-1", "F:B,H:1-1,C:5-2",
         * "F:B,H:1-1,C:5-3", "F:B,H:1-1,C:6-1", "F:B,H:1-1,C:6-2", "F:B,H:1-1,C:7-1", "F:B,H:1-2,C:1-2",
         * "F:B,H:1-2,C:1-3", "F:B,H:1-2,C:1-4", "F:B,H:1-2,C:1-5", "F:B,H:1-2,C:2-2", "F:B,H:1-2,C:2-3",
         * "F:B,H:1-2,C:2-4", "F:B,H:1-2,C:2-5", "F:B,H:1-2,C:3-2", "F:B,H:1-2,C:3-3", "F:B,H:1-2,C:3-4",
         * "F:B,H:1-2,C:3-5", "F:B,H:1-2,C:4-2", "F:B,H:1-2,C:4-3", "F:B,H:1-2,C:4-4", "F:B,H:1-2,C:5-2",
         * "F:B,H:1-2,C:5-3", "F:B,H:1-2,C:6-2", "F:B,H:1-3,C:1-3", "F:B,H:1-3,C:1-4", "F:B,H:1-3,C:1-5",
         * "F:B,H:1-3,C:1-6", "F:B,H:1-3,C:2-3", "F:B,H:1-3,C:2-4", "F:B,H:1-3,C:2-5", "F:B,H:1-3,C:3-3",
         * "F:B,H:1-3,C:3-4", "F:B,H:1-3,C:3-5", "F:B,H:1-3,C:4-3", "F:B,H:1-3,C:4-4", "F:B,H:1-4,C:1-4",
         * "F:B,H:1-4,C:2-4", "F:B,H:1-4,C:1-5", "F:B,H:2-1,C:2-1", "F:B,H:2-1,C:2-2", "F:B,H:2-1,C:2-3",
         * "F:B,H:2-1,C:2-4", "F:B,H:2-1,C:3-1", "F:B,H:2-1,C:3-2", "F:B,H:2-1,C:3-3", "F:B,H:2-1,C:3-4",
         * "F:B,H:2-1,C:4-1", "F:B,H:2-1,C:4-2", "F:B,H:2-1,C:4-3", "F:B,H:2-1,C:4-4", "F:B,H:2-1,C:5-1",
         * "F:B,H:2-1,C:5-2", "F:B,H:2-1,C:5-3", "F:B,H:2-1,C:5-6", "F:B,H:2-1,C:6-1", "F:B,H:2-1,C:6-2",
         * "F:B,H:2-1,C:6-3", "F:B,H:2-1,C:7-1", "F:B,H:2-1,C:7-2", "F:B,H:2-2,C:2-2", "F:B,H:2-2,C:2-3",
         * "F:B,H:2-2,C:2-4", "F:B,H:2-2,C:3-2", "F:B,H:2-2,C:3-3", "F:B,H:2-2,C:3-4", "F:B,H:2-2,C:3-5",
         * "F:B,H:2-2,C:4-2", "F:B,H:2-2,C:4-3", "F:B,H:2-2,C:4-4", "F:B,H:2-2,C:5-2", "F:B,H:2-2,C:5-3",
         * "F:B,H:2-2,C:5-4", "F:B,H:2-2,C:6-2", "F:B,H:2-3,C:2-3", "F:B,H:2-3,C:2-4", "F:B,H:2-3,C:3-3",
         * "F:B,H:2-3,C:3-4", "F:B,H:2-3,C:3-5", "F:B,H:2-3,C:4-3", "F:B,H:2-3,C:4-4", "F:B,H:2-2,C:4-5",
         * "F:B,H:2-3,C:5-3", "F:A,H:2-3,C:5-4", "F:B,H:2-4,C:3-4", "F:B,H:3-1,C:3-1", "F:B,H:3-1,C:3-2",
         * "F:B,H:3-1,C:3-3", "F:B,H:3-1,C:4-1", "F:B,H:3-1,C:4-2", "F:B,H:3-1,C:4-3", "F:B,H:3-1,C:4-4",
         * "F:A,H:3-1,C:4-5", "F:B,H:3-1,C:5-1", "F:B,H:3-1,C:5-2", "F:B,H:3-1,C:5-3", "F:B,H:3-1,C:6-1",
         * "F:B,H:3-1,C:6-2", "F:B,H:3-1,C:7-1", "F:B,H:3-2,C:3-2", "F:B,H:3-2,C:3-3", "F:B,H:3-2,C:3-4",
         * "F:B,H:3-2,C:4-2", "F:B,H:3-2,C:4-3", "F:B,H:3-2,C:4-4", "F:B,H:3-2,C:4-5", "F:B,H:3-2,C:5-2",
         * "F:B,H:3-2,C:5-3", "F:B,H:3-2,C:6-2", "F:B,H:3-2,C:6-3", "F:B,H:3-3,C:3-3", "F:B,H:3-4,C:4-4",
         * "F:B,H:3-4,C:5-4", "F:B,H:3-3,C:4-3", "F:B,H:3-3,C:4-4", "F:B,H:4-1,C:4-1", "F:B,H:4-1,C:4-2",
         * "F:B,H:4-1,C:5-1", "F:B,H:4-1,C:5-2", "F:B,H:4-1,C:6-1", "F:B,H:4-1,C:6-2", "F:B,H:4-1,C:7-1",
         * "F:B,H:4-2,C:4-2", "F:B,H:4-2,C:5-2", "F:B,H:4-2,C:6-2", "F:B,H:4-3,C:4-3", "F:B,H:5-1,C:6-1",
         * "F:B,H:0-5,C:0-7", "F:X,H:0-0,C:0-0");
         */
        final int maxFullHome = 9;
        final int maxFullAway = 8;
        final int maxHalfHome = 6;
        final int maxHalfAway = 5;
        final int maxFullTotal = 10;
        final int maxHalfTotal = 8;
        if (babSelection.size() == 0) {
            for (int fh = 0; fh < maxFullHome; fh++)
                for (int fa = 0; fa < maxFullAway; fa++)
                    for (int hh = 0; hh <= Math.min(fh, maxHalfHome); hh++)
                        for (int ha = 0; ha <= Math.min(fa, maxHalfAway); ha++) {
                            // max "F:A,H:6-1,C:7-1",
                            if (fh + fa <= maxFullTotal && hh + ha <= maxHalfTotal) {
                                // A first score
                                if (hh == 0 && ha == 0 && fh > 0)
                                    babSelection.add("F:A,H:" + hh + "-" + ha + ",C:" + fh + "-" + fa);
                                if (hh > 0 && fh > 0)
                                    babSelection.add("F:A,H:" + hh + "-" + ha + ",C:" + fh + "-" + fa);
                                // B first score
                                if (hh == 0 && ha == 0 && fa > 0)
                                    babSelection.add("F:B,H:" + hh + "-" + ha + ",C:" + fh + "-" + fa);
                                if (ha > 0 && fa > 0)
                                    babSelection.add("F:B,H:" + hh + "-" + ha + ",C:" + fh + "-" + fa);
                            }
                        }
            // add 0-0-0-0
            babSelection.add("F:X,H:0-0,C:0-0");
        }
    }
