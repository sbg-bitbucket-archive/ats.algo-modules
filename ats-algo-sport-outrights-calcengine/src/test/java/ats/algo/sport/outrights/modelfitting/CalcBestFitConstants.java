...
public class CalcBestFitConstants {
    ...
    public double[] calcBestFitConstants4Params() {
        ...
        System.out.printf(
                        "success: %b, #iterations: %d, attackRatingFactor: %.3f, defenseRatingFactor: %.3f, homeAdjFactor: %.3f, awayAdjFactor: %.3f",
    ...
    public double calcErrCostOnly(double[] x, Map<String, HistoryMatchInfos> historyMap,
        ...
        try {
            ...
            sb.append((title1 + title2 + "Bet 365, Us, " + title3 + title4 + title5 + title6) + ('\n'));
            ...
            for (FiveThirtyEightPrediction q : predictions) {
                ...
                if (selectL2NormOtherwiseL1Norm) {
                    ...
                    if (compareProbsTrueOtherwiseComparePrices)
                        ...
                        sumSqErr = Math.abs(1. / (q.probHomeWin + baseProb) - 1. / (p.getProbHomeWin() + baseProb))
                                        + Math.abs(1. / (q.probAwayWin + baseProb)
                                                        - 1. / (p.getProbAwayWin() + baseProb))
                                        + Math.abs(1. / (q.probDraw + baseProb) - 1. / (p.getProbDraw() + baseProb));
                }
                ...
                sb.append(nameKey.append(',')
                                ...
                                .append(',').append("Bet365,Us").append(',').append(q.getProbHomeWin()).append(',')
                                .append(p.getProbHomeWin()).append(',').append(q.getProbAwayWin()).append(',')
                                .append(p.getProbAwayWin()).append(',').append(sumSqErr).append('\n'));
    ...
    private void residualErrors(String leagueName) {
        ...
        System.out.println("4 params model cost: " + calcErrCostOnly(x, historyMap, l2NormOtherwiseL1Norm,
                        league + " 4-params-model-" + "L2 Norm-" + l2NormOtherwiseL1Norm + "-Compare Probs-"
                                        + compareProbsTrueOtherwiseComparePrices));
        ...
        x[3] = -bestTwoParams[1];
        System.out.println("2 param model cost (homeAdvantage: " + bestTwoParams[0] + ", meanAdj: " + bestTwoParams[1]
                        + "): "
    ...
    private void loadMap(String league) {
        try {
            historyMap = OutrightsHyperparamsfind.loadDataForMatchPredictionTestUse(league);
        } catch (IOException e) {
            e.printStackTrace();
        } catch (ParseException e) {
            e.printStackTrace();
