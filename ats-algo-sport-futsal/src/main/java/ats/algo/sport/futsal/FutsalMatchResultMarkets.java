...
public class FutsalMatchResultMarkets extends MatchResultMarkets {

    FutsalMatchState previousMatchState;
    FutsalMatchState currentMatchState;

    @Override
    protected CheckMarketResultedOutcome checkMarketResulted(Market market, MatchState previousMatchState,
                    MatchState currentMatchState) {
        this.previousMatchState = (FutsalMatchState) previousMatchState;
        this.currentMatchState = (FutsalMatchState) currentMatchState;
        ...
        switch (market.getType()) {
            case "FT:AXB": // Match winner
                ...
                if (matchWinner != null) {
                    ...
                    return null;
            case "FT:TOFG": // Match winner
                if (this.currentMatchState.isMatchCompleted() || this.currentMatchState.getGoalNo() > 1) {
                    int firstGoalIndex = this.currentMatchState.getElapsedTimeSecsFirstGoal();
                    if (firstGoalIndex != -1)
                        winningSelection = String.format("%d", 1 + firstGoalIndex / 600);
                    else
            ...
            case "FT:5MR": // 5 mins Match winner
                ...
                // int fiveMinsMarketSeqNo =
                // convertGoalSeqIdToIndex(market.getSequenceId());
                if (matchWinner != null) {
                    switch (matchWinner) {
                        case A:
                            winningSelection = "A";
            ...
            case "FTOT:CS": // correct score
                ...
                // case "FT:TOFG": // time first goal
                // int timeFirstGoal =
                // this.currentMatchState.getElapsedTimeSecsFirstGoal();
                // if (timeFirstGoal != 0) {
            ...
            default:
                throw new IllegalArgumentException(
                                "Market type missing from FutsalMatchResultMarkets: " + market.getType());
