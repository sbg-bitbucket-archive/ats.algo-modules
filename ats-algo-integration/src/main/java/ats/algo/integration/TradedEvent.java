...
public final class TradedEvent extends AtsBean {
    ...
    public TradedEvent(MatchDetails match) {
        ...
        if (match.isOutright() != null && match.isOutright()) {
            debug("Creating Traded event - Outright event: " + eventId);
            sportType = SupportedSportType.OUTRIGHTS;
        } else {
            debug("Creating Traded event - Sport event: " + eventId);
    ...
    public void setCurrentParams(GenericMatchParams matchParams) {
    ...
    public NotificationMessage createDisplayVariable(String dvName, String dvValue) {
        NotificationMessage dv = createNotificationMessage("DisplayVariable");
        dv.setName(dvName);
        dv.setDescription(dvValue);
        return dv;
    }
