...
public class TabletennisMatchParams extends MatchParams {

    private static final long serialVersionUID = 1L;
    private MatchParam onServePctA;
    private MatchParam onServePctB;

    public TabletennisMatchParams() {
        setDefaultParams();
    }

    @JsonCreator
    private TabletennisMatchParams(Map<String, Object> delegate) {
        super(delegate);
    ...
    public void updateParamsGivenMatchIncidentResult(MatchIncidentResult matchIncidentResult) {
        ...
        TeamId teamId = ((TabletennisMatchIncidentResult) matchIncidentResult).getTeamId();
        switch (((TabletennisMatchIncidentResult) matchIncidentResult).getTabletennisMatchIncidentResultType()) {
            case POINTWON:
                if (TeamId.A == teamId)
                    playerAWonLeg = true;
                else
        ...
        serveA = ((TabletennisMatchIncidentResult) matchIncidentResult).isPlayerAServedPoint();
    ...
    public void setDefaultParams(MatchFormat matchFormat) {
        onServePctA = new MatchParam(MatchParamType.A, MarketGroup.NOT_SPECIFIED, .5, 0.05, 0, 1, true);
        onServePctB = new MatchParam(MatchParamType.B, MarketGroup.NOT_SPECIFIED, .5, 0.05, 0, 1, true);
        updateParamMap();
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        TabletennisMatchParams other = (TabletennisMatchParams) obj;
        if (onServePctA == null) {
