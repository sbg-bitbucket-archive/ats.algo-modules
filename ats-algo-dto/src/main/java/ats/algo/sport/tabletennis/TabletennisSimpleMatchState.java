...
public class TabletennisSimpleMatchState extends SimpleMatchState {
     ...
     */
    @JsonCreator
    public TabletennisSimpleMatchState(@JsonProperty("preMatch") boolean preMatch,
    ...
    public TabletennisSimpleMatchState() {
        super();
            ...
            if (setScore[i].A > (-1) && setScore[i].B > (-1)) {
                ...
                switch (i + 1) {
                    ...
                    case 6:
                        scoreA = Integer.toString(setScore[i].A);
                        scoreB = Integer.toString(setScore[i].B);
                        resultMap.put("set6Score", new MatchResultElement(MatchResultEntryType.INT_HYPHEN_INT, 0, 50,
                                        scoreA + "-" + scoreB));
                        break;
                    case 7:
                        scoreA = Integer.toString(setScore[i].A);
                        scoreB = Integer.toString(setScore[i].B);
                        resultMap.put("set7Score", new MatchResultElement(MatchResultEntryType.INT_HYPHEN_INT, 0, 50,
                                        scoreA + "-" + scoreB));
                        break;
                }
            }
    ...
    @Override
    public TabletennisSimpleMatchState generateMatchStateFromMatchResultMap(MatchResultMap result,
                    MatchFormat matchFormat) {
        TabletennisMatchState tms = new TabletennisMatchState(matchFormat);
        ...
        int nSets = tableTennisMatchFormat.getnGamesInMatch();
        PairOfIntegers[] setScore = new PairOfIntegers[nSets];
        ...
        if (nSets == 7) {
            if (map.get("set7Score") != null)
                setScore[6] = map.get("set7Score").valueAsPairOfIntegers();
            else
                nSets--;
            if (map.get("set6Score") != null)
                setScore[5] = map.get("set6Score").valueAsPairOfIntegers();
            else
                nSets--;
            if (map.get("set5Score") != null)
                setScore[4] = map.get("set5Score").valueAsPairOfIntegers();
        ...
        tms.setGamesA(gamesA);
        tms.setGamesB(gamesB);

        TabletennisSimpleMatchState sms = tms.generateSimpleMatchState();
        return sms;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        TabletennisSimpleMatchState other = (TabletennisSimpleMatchState) obj;
