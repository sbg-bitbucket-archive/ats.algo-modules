...
public class FloorballMatchState extends AlgoMatchState {
    /* CJ added for penalties condition */

    /**
     * AlgoMatchState class for Floorball
     * 
     * @author Jin
     *
     */
    ...
    private FloorballMatchPeriod matchPeriodInWhichLastGoalScored;
    ...
    private FloorballMatchPeriod matchPeriod; // the state following the most
    private final FloorballMatchFormat matchFormat;
     ...
     */
    public FloorballMatchState() {
        this(new FloorballMatchFormat());
        // super();
        ...
        // matchPeriod = FloorballMatchPeriod.IN_SECOND_PERIOD;
     ...
     * @param matchFormat
     */
    public FloorballMatchState(MatchFormat matchFormat) {
        super();
        this.matchFormat = (FloorballMatchFormat) matchFormat;
    ...
    public void setElapsedTimeSecs(int elapsedTimeSecs, int overtimeNoIn) {
        ...
        matchPeriod = FloorballMatchPeriod.IN_FIRST_PERIOD;
        if (elapsedTimeSecs == normalPeriodSecs) {
            this.elapsedTimeThisPeriodSecs = 0;
            matchPeriod = FloorballMatchPeriod.AT_FIRST_PERIOD_END;
        }
        if (elapsedTimeSecs > normalPeriodSecs) {
        ...
        if (elapsedTimeSecs == 2 * normalPeriodSecs) {
            this.elapsedTimeThisPeriodSecs = 0;
            matchPeriod = FloorballMatchPeriod.AT_SECOND_PERIOD_END;
        }
        if (elapsedTimeSecs > 2 * normalPeriodSecs) {
            this.elapsedTimeThisPeriodSecs = elapsedTimeSecs - 2 * normalPeriodSecs;
            matchPeriod = FloorballMatchPeriod.IN_THIRD_PERIOD;
        }
        if (elapsedTimeSecs == 3 * normalPeriodSecs) {
            this.elapsedTimeThisPeriodSecs = 0;
            matchPeriod = FloorballMatchPeriod.AT_FULL_TIME;
        }
        if (elapsedTimeSecs > 3 * normalPeriodSecs) {
            this.elapsedTimeThisPeriodSecs = elapsedTimeSecs - overtimeNoIn * extraPeriodSecs;
            matchPeriod = FloorballMatchPeriod.IN_EXTRA_TIME;
        }
    ...
    public MatchIncidentResult updateStateForIncident(MatchIncident matchIncident,
        ...
        if (matchIncident.getClass() == FloorballMatchIncident.class) {

            TeamId teamId = ((FloorballMatchIncident) matchIncident).getTeamId();

            switch (((FloorballMatchIncident) matchIncident).getIncidentSubType()) {
                case MINOR_PENALTY_END:
                    switch (teamId) {
                        case A:
                            if (minorSinBinA == 0)
                                throw new IllegalArgumentException("Send-Off number is wrong");
                            minorSinBinA--;
                            cleanSuspensionTimer(minorSinBinTimer_A, minorSinBinA);
                            break;
                        case B:
                            if (minorSinBinB == 0)
                                throw new IllegalArgumentException("Send-Off number is wrong");
                            minorSinBinB--;
                            cleanSuspensionTimer(minorSinBinTimer_B, minorSinBinB);
                            break;
                        default:
                ...
                case GOAL:
                    ...
                    switch (teamId) {
                        case A:
                            if (!matchPeriod.equals(FloorballMatchPeriod.IN_SHOOTOUT)) {

                                if (!matchPeriod.equals(FloorballMatchPeriod.IN_EXTRA_TIME))
                                    normalTimeGoalsA++;
                                ...
                                if (matchPeriod.equals(FloorballMatchPeriod.IN_FIRST_PERIOD))
                                    goalsFirstPeriodA++;
                                if (matchPeriod.equals(FloorballMatchPeriod.IN_SECOND_PERIOD))
                                    goalsSecondPeriodA++;
                                if (matchPeriod.equals(FloorballMatchPeriod.IN_THIRD_PERIOD))
                                    goalsThirdPeriodA++;
                                ...
                                if (isShootoutSettled()) {
                                    ...
                                    matchPeriod = FloorballMatchPeriod.MATCH_COMPLETED;
                                }
            ...
            switch (((ElapsedTimeMatchIncident) matchIncident).getIncidentSubType()) {
                    ...
                    switch (matchPeriod) {
                        ...
                        case AT_EXTRA_PERIOD_END:
                            ...
                            if (penaltiesPossible && goalsA == goalsB) {
                                matchPeriod = FloorballMatchPeriod.IN_SHOOTOUT;
                                break;
                            } else {
                                if (goalsA != goalsB) {
                                    matchPeriod = FloorballMatchPeriod.MATCH_COMPLETED;
                ...
                case SET_PERIOD_END:
                    switch (matchPeriod) {
                        ...
                        case IN_EXTRA_TIME:
                            if (goalsA == goalsB) {
                                matchPeriod = FloorballMatchPeriod.AT_EXTRA_PERIOD_END;
                            } else {
                                matchPeriod = FloorballMatchPeriod.MATCH_COMPLETED;
                            }
                            previousPeriodGoalsA = currentPeriodGoalsA;
    ...
    public AlgoMatchState copy() {
        FloorballMatchState cc = new FloorballMatchState(matchFormat);
        cc.setEqualTo(this);
        return cc;
    ...
    public void setEqualTo(AlgoMatchState matchState) {
        ...
        this.setGoalsA(((FloorballMatchState) matchState).getGoalsA());
        this.setGoalsB(((FloorballMatchState) matchState).getGoalsB());
        this.setElapsedTimeSecs(((FloorballMatchState) matchState).getElapsedTimeSecs(),
                        ((FloorballMatchState) matchState).getOvertimeNo());
        this.setMatchPeriod(((FloorballMatchState) matchState).getMatchPeriod());
        this.setTeamScoringLastGoal(((FloorballMatchState) matchState).getTeamScoringLastGoal());
        /* update cards infor and sendoff info */
        this.minorSinBinTimer_A.clear();
        this.minorSinBinTimer_A = cloneIntegerList(((FloorballMatchState) matchState).getMinorSinBinTimer_A());

        this.minorSinBinTimer_B.clear();
        this.minorSinBinTimer_B = cloneIntegerList(((FloorballMatchState) matchState).getMinorSinBinTimer_B());

        this.majorSinBinTimer_A.clear();
        this.majorSinBinTimer_A = cloneIntegerList(((FloorballMatchState) matchState).getMajorSinBinTimer_A());

        this.majorSinBinTimer_B.clear();
        this.majorSinBinTimer_B = cloneIntegerList(((FloorballMatchState) matchState).getMajorSinBinTimer_B());

        this.tenMinsSinBinTimer_A.clear();
        this.tenMinsSinBinTimer_A = cloneIntegerList(((FloorballMatchState) matchState).getTenMinsSinBinTimer_A());

        this.tenMinsSinBinTimer_B.clear();
        this.tenMinsSinBinTimer_B = cloneIntegerList(((FloorballMatchState) matchState).getTenMinsSinBinTimer_B());

        this.setTenMinsSinBinA(((FloorballMatchState) matchState).getTenMinsSinBinA());
        this.setTenMinsSinBinB(((FloorballMatchState) matchState).getTenMinsSinBinB());
        this.setMajorSinBinA(((FloorballMatchState) matchState).getMajorSinBinA());
        this.setMajorSinBinB(((FloorballMatchState) matchState).getMajorSinBinB());
        this.setSendOffA(((FloorballMatchState) matchState).getMinorSinBinA());
        this.setSendOffB(((FloorballMatchState) matchState).getMinorSinBinB());
        // minor sin bin
        this.setShootOutGoalsA(((FloorballMatchState) matchState).getShootOutGoalsA());
        this.setShootOutGoalsB(((FloorballMatchState) matchState).getShootOutGoalsB());
        this.setShootOutTimeCounter(((FloorballMatchState) matchState).getShootOutTimeCounter());
        this.setOvertimeNo(((FloorballMatchState) matchState).getOvertimeNo());
        this.setGoalSequence(((FloorballMatchState) matchState).getGoalSequence());

        this.setCurrentPeriodGoalsA(((FloorballMatchState) matchState).getCurrentPeriodGoalsA());
        this.setCurrentPeriodGoalsB(((FloorballMatchState) matchState).getCurrentPeriodGoalsB());
        this.setOvertimeNo(((FloorballMatchState) matchState).getOvertimeNo());
        this.setNormalTimeGoalsA(((FloorballMatchState) matchState).getNormalTimeGoalsA());
        this.setNormalTimeGoalsB(((FloorballMatchState) matchState).getNormalTimeGoalsB());
        this.setElapsedTimeSecsFirstGoal(((FloorballMatchState) matchState).getElapsedTimeSecsFirstGoal());

        this.setElapsedTimeThisPeriodSecs(((FloorballMatchState) matchState).getElapsedTimeThisPeriodSecs());
        this.setElapsedTime(((FloorballMatchState) matchState).getElapsedTimeSecs());

        this.setGoalsSecondPeriodA(((FloorballMatchState) matchState).getGoalsSecondPeriodA());
        this.setGoalsSecondPeriodB(((FloorballMatchState) matchState).getGoalsSecondPeriodB());

        this.setGoalsThirdPeriodA(((FloorballMatchState) matchState).getGoalsThirdPeriodA());
        this.setGoalsThirdPeriodB(((FloorballMatchState) matchState).getGoalsThirdPeriodB());
        this.setFiveMinsGoalsA(((FloorballMatchState) matchState).getFiveMinsGoalsA());
        this.setFiveMinsGoalsB(((FloorballMatchState) matchState).getFiveMinsGoalsB());
        this.setPreviousFiveMinsGoalsA(((FloorballMatchState) matchState).getPreviousFiveMinsGoalsA());
        this.setPreviousFiveMinsGoalsB(((FloorballMatchState) matchState).getPreviousFiveMinsGoalsB());
        this.setCurrentFiveMinsEnder(((FloorballMatchState) matchState).getCurrentFiveMinsEnder());
        this.setPreviousFiveMinsEnder(((FloorballMatchState) matchState).getPreviousFiveMinsEnder());
        this.setFiveMinsNo(((FloorballMatchState) matchState).getFiveMinsNo());
    ...
    public MatchIncidentPrompt getNextPrompt() {
        ...
        if (matchPeriod == FloorballMatchPeriod.MATCH_COMPLETED)
            matchIncidentPrompt = new MatchIncidentPrompt("Match finished");
        else if (matchPeriod == FloorballMatchPeriod.PREMATCH || matchPeriod == FloorballMatchPeriod.AT_FIRST_PERIOD_END
                        || matchPeriod == FloorballMatchPeriod.AT_SECOND_PERIOD_END
                        || matchPeriod == FloorballMatchPeriod.AT_FULL_TIME
                        || matchPeriod == FloorballMatchPeriod.AT_EXTRA_PERIOD_END)
            ...
            matchIncidentPrompt = new MatchIncidentPrompt(
                            "Next event (Nnn-Nothing happened for nn periods, H-Home goal, A-Away goal, A-Away goal, GH/A-Home/Away 2 minutes send off, "
                                            ...
                                            + "RH/A-Home/Away 10 minutes send off,"
                                            + "TH/A,QH/A,UH/A- end send off home or away, E- end period)",
                            "N");
    ...
    public MatchIncident getMatchIncident(String response) {
        ...
        FloorballMatchIncidentType floorballMatchIncidentType = null;
        ...
        switch (c) {
            case 'N':
                ...
                break;
            case 'H':
                floorballMatchIncidentType = FloorballMatchIncidentType.GOAL;
                teamId = TeamId.A;
                break;
            case 'A':
                floorballMatchIncidentType = FloorballMatchIncidentType.GOAL;
            ...
            case 'G':// green card
                ...
                try {
                    ...
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        floorballMatchIncidentType = FloorballMatchIncidentType.MINOR_PENALTY_START;
                        teamId = TeamId.A;
            ...
            case 'Y':// yellow card
                ...
                try {
                    String teamCount = (response.substring(1));
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        floorballMatchIncidentType = FloorballMatchIncidentType.MAJOR_PENALTY_START;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
            ...
            case 'R':// red card 10 mins
                ...
                try {
                    String teamCount = (response.substring(1));
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        floorballMatchIncidentType = FloorballMatchIncidentType.TEN_MINS_PENALTY_START;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
            ...
            case 'U':// terminate major sin bin
                ...
                try {
                    String teamCount = (response.substring(1));
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        floorballMatchIncidentType = FloorballMatchIncidentType.TEN_MINS_PENALTY_END;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
            ...
            case 'T':// terminate minor sin bin
                ...
                try {
                    String teamCount = (response.substring(1));
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        floorballMatchIncidentType = FloorballMatchIncidentType.MINOR_PENALTY_END;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
            ...
            case 'Q':// terminate major sin bin
                ...
                try {
                    String teamCount = (response.substring(1));
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        floorballMatchIncidentType = FloorballMatchIncidentType.MAJOR_PENALTY_END;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
        ...
        if (floorballMatchIncidentType != null) {
            incident = new FloorballMatchIncident(floorballMatchIncidentType, incidentElapsedTimeSecs, teamId);
        } else {
    ...
    public boolean isMatchCompleted() {
        return (matchPeriod == FloorballMatchPeriod.MATCH_COMPLETED);
    }
    ...
    public boolean isPeriodCompleted() {
        return (matchPeriod == FloorballMatchPeriod.AT_FIRST_PERIOD_END
                        || matchPeriod == FloorballMatchPeriod.AT_SECOND_PERIOD_END
                        || matchPeriod == FloorballMatchPeriod.MATCH_COMPLETED);
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        FloorballMatchState other = (FloorballMatchState) obj;
    ...
    public boolean preMatch() {
        return (matchPeriod == FloorballMatchPeriod.PREMATCH);
    }
    ...
    public SimpleMatchState generateSimpleMatchState() {
        return new FloorballSimpleMatchState(this.preMatch(), this.isMatchCompleted(), matchPeriod, elapsedTimeSecs,
                        goalsA, goalsB, goalsFirstPeriodA, goalsFirstPeriodB, goalsSecondPeriodA, goalsSecondPeriodB,
                        goalsThirdPeriodA, goalsThirdPeriodB);
    }
