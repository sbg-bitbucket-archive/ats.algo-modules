...
public class RugbyLeagueMatchMarketsFactory extends MarketsFactory {

    /**
     * holds any facts about a simulation of a match that has just been played (via the Match class) that are need in
     * the calculation of the required statistics and that are not captured by the matchState object for RugbyLeague if
    ...
    RugbyLeagueMatchMarketsFactory(RugbyLeagueMatchState matchState) {
        ...
        trysTotalCurrentPeriod = new TotalStatistic("P:OU", marketDescription, MarketGroup.GOALS, generateMarketFT,
        ...
        periodCorrectTrys = new CorrectScoreStatistic("P:CS", marketDescription, MarketGroup.GOALS, generateMarketFT,
                        periodSequenceId, maxNoGoalsPerTeam);
        ...
        generateMarketFT = (matchState.getMatchPeriod() != RugbyLeagueMatchPeriod.AT_HALF_TIME
                        && matchState.getMatchPeriod() != RugbyLeagueMatchPeriod.AT_FULL_TIME
                        && matchState.getMatchPeriod() != RugbyLeagueMatchPeriod.IN_EXTRA_TIME_FIRST_HALF
                        && matchState.getMatchPeriod() != RugbyLeagueMatchPeriod.IN_EXTRA_TIME_SECOND_HALF
                        && matchState.getMatchPeriod() != RugbyLeagueMatchPeriod.AT_EXTRA_TIME_HALF_TIME
                        && matchState.getMatchPeriod() != RugbyLeagueMatchPeriod.AT_EXTRA_TIME_END
                        && matchState.getMatchPeriod() != RugbyLeagueMatchPeriod.IN_SHOOTOUT
    ...
    void updateStats(RugbyLeagueMatchState matchState, RugbyLeagueMatchFacts matchFacts) {
