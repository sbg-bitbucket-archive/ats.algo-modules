...
public class RugbyLeagueMatchResultMarkets extends MatchResultMarkets {

    RugbyLeagueMatchState previousMatchState;
    RugbyLeagueMatchState currentMatchState;

    @Override
    protected CheckMarketResultedOutcome checkMarketResulted(Market market, MatchState previousMatchState,
                    MatchState currentMatchState) {
        this.previousMatchState = (RugbyLeagueMatchState) previousMatchState;
        this.currentMatchState = (RugbyLeagueMatchState) currentMatchState;
        ...
        switch (market.getType()) {
            ...
            case "FT:NT": // next goal
                marketSeqNo = convertGoalSeqIdToIndex(market.getSequenceId());
                currentStateSeqNo = convertGoalSeqIdToIndex(this.currentMatchState.getSequenceIdForTry(0));
                if (currentStateSeqNo <= marketSeqNo) {
                    /*
                     * next goal has not yet been scored. see if match is over
                     */
                    if (this.currentMatchState.isNormalTimeMatchCompleted()) {
                        return new CheckMarketResultedOutcome("No Try");
                    } else
                        return null;
                }

                previousStateSeqNo = convertGoalSeqIdToIndex(this.previousMatchState.getSequenceIdForTry(0));
                if (previousStateSeqNo == marketSeqNo) {
                    if (this.currentMatchState.getTeamTryLastPoint() == TeamId.A)
                        winningSelection = "A";
                    else
                        winningSelection = "B";
                    return new CheckMarketResultedOutcome(winningSelection);
                } else {
                    throw new IllegalArgumentException("Only one Try should be scored in each matchState change");
                }
            ...
            case "P:TRYCS": // correct score
                ...
                previousPeriodSeqNo = convertGoalSeqIdToIndex(this.previousMatchState.getSequenceIdForPeriod(0));
                if (previousPeriodSeqNo == periodSeqNo) {
                    winningSelection = String.format("%d-%d", this.currentMatchState.getCurrentPeriodTrysA(),
                                    this.currentMatchState.getCurrentPeriodTrysB());
                    return new CheckMarketResultedOutcome(winningSelection);
                } else {
                    throw new IllegalArgumentException("period correct score not resulting correctly");
                }

            case "P:TRYOU": // total goals in current half
                periodSeqNo = convertPeriodSeqIdToIndex(market.getSequenceId());
                currentPeriodSeqNo = convertPeriodSeqIdToIndex(this.currentMatchState.getSequenceIdForPeriod(0));
                if (currentPeriodSeqNo <= periodSeqNo) {
                    /*
                ...
                previousPeriodSeqNo = convertGoalSeqIdToIndex(this.previousMatchState.getSequenceIdForPeriod(0));
                if (previousPeriodSeqNo == periodSeqNo) {
                    int n = this.currentMatchState.getCurrentPeriodTrysA()
                                    + this.currentMatchState.getCurrentPeriodTrysB();
                    winningSelection = MarketUtilityFunctions.getWinningSelectionOvunMarket(n, market.getLineId());
                    return new CheckMarketResultedOutcome(winningSelection, n);
                } else {
            ...
            default:
                throw new IllegalArgumentException(
                                "Market type missing from RugbyLeagueMatchResultMarkets: " + market.getType());
