...
public class JmsAlgoManagerConfiguration extends AlgoManagerConfiguration
    ...
    private double priceCalcTimeoutSecs = 3600;
    private double priceCalcTimeoutMultiplier = 1;
    private double paramFindTimeoutSecs = 360;
    ...
    protected void schedulePriceCalcTimeoutWatch(PriceCalcRequest pcr) {
        ...
        if (pendingCount % 10 == 0) {
            debug("%s inflight PCRs. Timeout is %s", pendingCount, priceCalcTimeoutSecs);
        }
        double watchAt = priceCalcTimeoutSecs + (pendingCount * priceCalcTimeoutMultiplier);
        debug("%s inflight PCRs. Timeout is %s. This is a test. Watch = %s. Pending = %s. TimeoutMult = %s",
                        pendingCount, priceCalcTimeoutSecs, watchAt, pendingCount, priceCalcTimeoutMultiplier);
    ...
    protected void executePCRTimedOutLogic(PriceCalcRequest pcr) {
        ...
        if (removed != null) {
            ...
            decrementPendingPriceCalcCounter();
            long pendingCount = pendingPCRequestCount.get();
            String errMsg = String.format(
                            "Watchdog PCR timeout for event %d request %s. Currently there are %s PCR inflight.  Timeout is %s",
                            removed.getEventId(), uniqueRequestId, pendingCount, priceCalcTimeoutSecs);
