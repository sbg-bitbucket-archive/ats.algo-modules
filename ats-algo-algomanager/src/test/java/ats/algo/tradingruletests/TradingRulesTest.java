...
public class TradingRulesTest extends AlgoManagerSimpleTestBase {
    ...
    public void testIgnoreBookiePricingInput() {
        ...
        tennisMatchParams.getOnServePctA1().setMean(0.6);
    ...
    public void testMarketGroupSuspensionForParamFindPrematch() {
        ...
        for (Market cornerMarket : this.publishedMarkets.getMarkets().values()) {
            if (cornerMarket.getType().equals("FT:COU")) {
                market = cornerMarket;
            }
        }
        ...
        for (Entry<String, MatchParam> matchParam : footballMatchParams.getParamMap().entrySet()) {
            if (matchParam.getKey().equals("goalTotal")) {
                matchParam.getValue().getGaussian().setMean(2.6);
            }
        }
        ...
        for (Market cornerMarket : this.publishedMarkets.getMarkets().values()) {
            if (cornerMarket.getType().equals("FT:COU")) {
                if (cornerMarket.getLineId().equals("+9.5")) {
                    market = cornerMarket;
                }
            }
        }
    ...
    @Test
    public void testFootballTradingRuleAlertsForLc() {
        MethodName.log();
        System.setProperty("paramFindTimeOverride", "true,football");
        System.setProperty("pricesChangeOverride", "true,football");
        ...
        priceCalcRequest.setEventId(eventId);
        priceCalcRequest.updateUniqueRequestId("Test123");
        logger.error("*** EXPECT THIS TEST TO GENERATE ERROR ***");
        algoManager.getEventDetails(eventId).setPriceCalcInProgress(priceCalcRequest);
        algoManager.handlePriceCalcError(eventId, "Test123", "Test to see if an alert will be published");
        logger.error("*** END OF GENERATED ERROR ***");
        boolean correctAlert = false;

        for (TraderAlertAttributes attributes : traderAlert.getAlertAttributes().values()) {
            if (attributes.getAttributes().equals("Test to see if an alert will be published")) {
                correctAlert = true;
            }
        }

        assertTrue(correctAlert);
    ...
    public void testFootballTradingRulesForLc() {
        ...
        // System.out.println(result.getActionReason());
        assertTrue(result.isParamFindScheduled());
        System.clearProperty("paramFindTimeOverride");
        System.clearProperty("pricesChangeOverride");
    }

    @Test
    public void testRugbyLeagueSuspendingRule() {
        MethodName.log();
        long eventId = 123456;
        int incidentCount = 1;
        RugbyLeagueTradingRules tradingRules = new RugbyLeagueTradingRules();
        algoManager.setTradingRules(SupportedSportType.RUGBY_LEAGUE, tradingRules);
        RugbyLeagueMatchFormat matchFormat = new RugbyLeagueMatchFormat();
        algoManager.handleNewEventCreation(SupportedSportType.RUGBY_LEAGUE, eventId, matchFormat);
        Map<String, String> properties = new HashMap<String, String>(1);
        ...
        elapsedTimeMatchIncident.setIncidentId("Inc" + incidentCount);
        incidentCount++;
        ...
        RugbyLeagueMatchIncident.setEventId(eventId);
        RugbyLeagueMatchIncident.setIncidentId("Inc" + incidentCount);
        algoManager.handleMatchIncident(RugbyLeagueMatchIncident, true);
        incidentCount++;
    ...
    @Test
    public void testSnookerSuspendingRule() {
        MethodName.log();
        long eventId = 12346;
        int incidentCount = 1;
        SnookerTradingRules tradingRules = new SnookerTradingRules();
        algoManager.setTradingRules(SupportedSportType.SNOOKER, tradingRules);
        SnookerMatchFormat matchFormat = new SnookerMatchFormat();
        algoManager.handleNewEventCreation(SupportedSportType.SNOOKER, eventId, matchFormat);
        Map<String, String> properties = new HashMap<String, String>(1);
        properties.put("eventTier", "2");
        algoManager.handleSetEventProperties(eventId, properties);
        ElapsedTimeMatchIncident elapsedTimeMatchIncident;
        ...
        snookerMatchIncident.set(SnookerMatchIncidentType.FRAMEWON, TeamId.B);
        snookerMatchIncident.setEventId(eventId);
        snookerMatchIncident.setIncidentId("Inc" + incidentCount);
        algoManager.handleMatchIncident(snookerMatchIncident, true);
        incidentCount++;
    ...
    @Test
    public void testRugbyUnionSuspendingRule() {
        MethodName.log();
        long eventId = 1234L;
        int incidentCount = 1;
        RugbyUnionTradingRules tradingRules = new RugbyUnionTradingRules();
        algoManager.setTradingRules(SupportedSportType.RUGBY_UNION, tradingRules);
        RugbyUnionMatchFormat matchFormat = new RugbyUnionMatchFormat();
        algoManager.handleNewEventCreation(SupportedSportType.RUGBY_UNION, eventId, matchFormat);
        Map<String, String> properties = new HashMap<String, String>(1);
        properties.put("eventTier", "2");
        algoManager.handleSetEventProperties(eventId, properties);
        ElapsedTimeMatchIncident elapsedTimeMatchIncident;
        ...
        rugbyUnionMatchIncident.setEventId(eventId);
        rugbyUnionMatchIncident.setIncidentId("Inc" + incidentCount);
        algoManager.handleMatchIncident(rugbyUnionMatchIncident, true);
        incidentCount++;
    ...
    @Test
    public void testSquashSuspendingRule() {
        MethodName.log();
        long eventId = 12346L;
        int incidentCount = 1;
        SquashTradingRules tradingRules = new SquashTradingRules();
        algoManager.setTradingRules(SupportedSportType.SQUASH, tradingRules);
        SquashMatchFormat matchFormat = new SquashMatchFormat();
        algoManager.handleNewEventCreation(SupportedSportType.SQUASH, eventId, matchFormat);
        Map<String, String> properties = new HashMap<String, String>(1);
        properties.put("eventTier", "2");
        algoManager.handleSetEventProperties(eventId, properties);
        ElapsedTimeMatchIncident elapsedTimeMatchIncident;
        ...
        for (int i = 0; i <= 10; i++) {
            ...
            squashMatchIncident.setEventId(eventId);
            squashMatchIncident.setIncidentId("Inc" + incidentCount);
            algoManager.handleMatchIncident(squashMatchIncident, true);
            incidentCount++;
        }
        /*
         * suspend when game score changed
         */
        assertTrue(this.publishedEventLevelShouldSuspend);
    }
    ...
    public void test() {
        ...
        long eventId = 11L;
        algoManager.handleNewEventCreation(SupportedSportType.TENNIS, eventId, tennisMatchFormat, 2);
