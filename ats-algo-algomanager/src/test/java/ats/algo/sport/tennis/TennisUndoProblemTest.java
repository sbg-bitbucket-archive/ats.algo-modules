...
public class TennisUndoProblemTest extends AlgoManagerSharedMemoryTestBase {
    ...
    public void testIncidentJumpUndo() {
        ...
        algoManager.handleMatchIncident(getIncident(eventId, "START", TennisMatchIncidentType.MATCH_STARTING, TeamId.A),
                        true);
        algoManager.handleMatchIncident(getIncident(eventId, "15-0", TennisMatchIncidentType.POINT_WON, TeamId.A),
                        true);
        /*
         * wait for these two incidents to get processed
         */
        Sleep.sleep(3);
        algoManager.handleMatchIncident(getIncident(eventId, "30-0", TennisMatchIncidentType.POINT_WON, TeamId.A),
                        true);
        algoManager.handleMatchIncident(
                        getIncident(eventId, "Challenge", TennisMatchIncidentType.CHALLENGER_BALLMARK, TeamId.A), true);
        algoManager.handleMatchIncident(
                        getIncident(eventId, "Challenge1", TennisMatchIncidentType.SERVING_ORDER, TeamId.A), true);
        ...
        System.out.println(simpleMatchState.formattedGeneralState());
        algoManager.handleUndoMatchIncident(eventId,
                        getIncident(eventId, "30-0", TennisMatchIncidentType.POINT_WON, TeamId.A));
        Sleep.sleep(5);
        simpleMatchState = (TennisSimpleMatchState) publishedMatchState;
        System.out.println(simpleMatchState.formattedGeneralState());
        algoManager.handleMatchIncident(getIncident(eventId, "15-15", TennisMatchIncidentType.POINT_WON, TeamId.B),
                        true);
        Sleep.sleep(5);
        simpleMatchState = (TennisSimpleMatchState) publishedMatchState;
        System.out.println(simpleMatchState.formattedGeneralState());
