...
public class TabletennisResultMarkets extends MatchResultMarkets {
    TabletennisMatchState previousMatchState;
    TabletennisMatchState currentMatchState;
    int marketSetNo;
    int marketPointNo;
    ...
    protected CheckMarketResultedOutcome checkMarketResulted(Market market, MatchState previousMatchState,
        ...
        this.previousMatchState = (TabletennisMatchState) previousMatchState;
        this.currentMatchState = (TabletennisMatchState) currentMatchState;
        ...
        switch (market.getType()) {
            ...
            case "P:RT":
                return checkRTMktResulted(market);
            case "P:OU":
    ...
    private CheckMarketResultedOutcome checkRTMktResulted(Market market) {
        int currentSetNo = this.currentMatchState.getGamesA() + this.currentMatchState.getGamesB();

        if (currentSetNo >= marketSetNo) {
            TeamId winner = this.currentMatchState.getRaceToGameNWinner(marketSetNo);
            if (winner == TeamId.A)
                return new CheckMarketResultedOutcome("A");
            else if (winner == TeamId.B)
    ...
    private CheckMarketResultedOutcome checkSetPointMktResulted(Market market, int n) {
        ...
        if ((currentSetNo == marketSetNo - 1) && (currentPointNo == marketPointNo)) {
            ...
            int pointsA1 = this.previousMatchState.getPointsA();
            int pointsB1 = this.previousMatchState.getPointsB();
