...
public class PpbMarket implements Serializable {
    ...
    private String lineId; // if category = "OVUN" or "HCAP" then the line
                           ...
                           // format, e.g. "4.5". Otherwise set to "" or
                           // null
    private String marketDescription; // text string useful for toString
                                      // method,
                                      // but not used by ATS
    ...
    private DerivedMarketDetails derivedMarketDetails; // will be null if
                                                       // not a
                                                       // derived market
     ...
     */
    public PpbMarket(MarketCategory category, String type, String sequenceId, String marketDescription) {
     ...
     */
    public PpbMarket(MarketCategory category, MarketGroup marketGroup, String type, String sequenceId,
    ...
    private void initialiseMarket(MarketCategory category, MarketGroup marketGroup, String type, String sequenceId,
        ...
        selections = new TreeMap<String, Double>();
    ...
    /**
     * set the set of selection names and associated probabilities
     * 
     * @param selections
     */
    ...
    /**
     * for ATS integration. Not for general use
     * 
     * @return
     */
    @JsonIgnore
    ...
    @JsonIgnore
    public MarketTypeDescriptor getDescriptor(boolean manageSingleRollingMarket) {
        if (manageSingleRollingMarket) {
            return new MarketTypeDescriptor(marketCategory, type, "", sequenceId);
        }
     ...
     * @return
     */
    PpbMarket generateRegularMarketForLineId(String lineIdStr) {
        boolean validMarketType = this.valid
                        && (this.marketCategory == MarketCategory.HCAP || this.marketCategory == MarketCategory.OVUN);
        if (!validMarketType)
            return null;
        PpbMarket market = new PpbMarket(this.getCategory(), this.getMarketGroup(), this.getType(),
                        this.getSequenceId(), this.getMarketDescription());
     ...
     */
    @JsonIgnore
    public PpbMarket generateEuropeanMarketForLineId(String marketType, String marketDescription, String lineIdStr) {
        boolean validMarketType = this.valid
                        && (this.marketCategory == MarketCategory.HCAP || this.marketCategory == MarketCategory.OVUN);
        if (!validMarketType)
            return null;
        ...
        PpbMarket market = new PpbMarket(newMarketCategory, this.getMarketGroup(), marketType, this.getSequenceId(),
                        marketDescription);
        market.setIsValid(true);
        market.setLineBase(this.lineBase);
        market.setLineProbs(this.lineProbs);
         ...
         */
        int lineIdentifier = MarketUtilityFunctions.convertLineIdStringToLineIndex(lineIdStr);
        int index; // the index into the probArray which represents the
                   // balanced
                   // line
     ...
     */
    @JsonIgnore
    public PpbMarket getMarketForLineId(String lineIdStr) {
    ...
    @JsonIgnore
    @Deprecated
    public PpbMarket[] getAllLineMarketsSpecifiedByTradingRules() {

        ArrayList<PpbMarket> mktList = new ArrayList<PpbMarket>();
        ...
        PpbMarket[] markets = new PpbMarket[mktList.size()];
     ...
     * @return
     */
    public PpbMarket copy() {
        PpbMarket marketCopy = new PpbMarket(this.getCategory(), this.getType(), this.getSequenceId(),
        ...
        for (Entry<String, Double> selEntry : selections.entrySet()) {
            marketCopy.getSelections().put(selEntry.getKey(), selEntry.getValue());
        }
        return marketCopy;
    }

    /**
     * generate the ppb market variant from the standard algo version. Only difference is selections property is Map
     * <String,Double> instead of Map<String, Selection>
     * 
     * @param market
     * @return
     */
    public static PpbMarket generatePpbMarket(Market market) {
        PpbMarket ppbMarket = new PpbMarket(market.getCategory(), market.getType(), market.getSequenceId(),
                        market.getMarketDescription());
        ppbMarket.setSelectionNameOverOrA(market.getSelectionNameOverOrA());
        ppbMarket.setSelectionNameUnderOrB(market.getSelectionNameUnderOrB());
        ppbMarket.setLineId(market.getLineId());
        ppbMarket.setIsValid(market.isValid());
        ppbMarket.setLineBase(market.getLineBase());
        ppbMarket.setLineProbs(market.getLineProbs());
        ppbMarket.setMarketGroup(market.getMarketGroup());
        ppbMarket.setMarketStatus(market.getMarketStatus().copy());
        DerivedMarketsInfo derivedMarketsInfo = market.getDerivedMarketsInfo();
        if (derivedMarketsInfo != null)
            ppbMarket.setDerivedMarketsInfo(derivedMarketsInfo.copy());
        for (Entry<String, Selection> selEntry : market.getSelections().entrySet()) {
            ppbMarket.getSelections().put(selEntry.getKey(), selEntry.getValue().getProb());
        }
        return ppbMarket;
    }

    public static Market generateAlgoMarket(PpbMarket ppbMarket) {
        Market market = new Market(ppbMarket.getCategory(), ppbMarket.getType(), ppbMarket.getSequenceId(),
                        ppbMarket.getMarketDescription());
        market.setSelectionNameOverOrA(ppbMarket.getSelectionNameOverOrA());
        market.setSelectionNameUnderOrB(ppbMarket.getSelectionNameUnderOrB());
        market.setLineId(ppbMarket.getLineId());
        market.setIsValid(ppbMarket.isValid());
        market.setLineBase(ppbMarket.getLineBase());
        market.setLineProbs(ppbMarket.getLineProbs());
        market.setMarketGroup(ppbMarket.getMarketGroup());
        market.setMarketStatus(ppbMarket.getMarketStatus().copy());
        DerivedMarketsInfo derivedMarketsInfo = ppbMarket.getDerivedMarketsInfo();
        if (derivedMarketsInfo != null)
            market.setDerivedMarketsInfo(derivedMarketsInfo.copy());
        for (Entry<String, Double> selEntry : ppbMarket.getSelections().entrySet()) {
            market.getSelections().put(selEntry.getKey(), new Selection(selEntry.getValue()));
        }
        return market;
    }
    ...
    public Double put(String key, Double value) {
        return selections.put(key, value);
    }
