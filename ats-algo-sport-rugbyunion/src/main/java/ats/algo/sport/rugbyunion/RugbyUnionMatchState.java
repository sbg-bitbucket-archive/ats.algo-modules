...
/**
 * AlgoMatchState class for Rugby Union
 * 
 * @author
 *
 */
@JsonIgnoreProperties(ignoreUnknown = true)
public class RugbyUnionMatchState extends AlgoMatchState {
    /**
     * 
     */
    ...
    @JsonIgnore
    private RugbyUnionMatchStatus rugbyUnionMatchStatus;
    ...
    private RugbyUnionMatchPeriod matchPeriodInWhichLastGoalScored;
    ...
    private RugbyUnionMatchPeriod matchPeriod; // the state following the most
    private final RugbyUnionMatchFormat matchFormat;
    ...
    public RugbyUnionMatchState() {
        ...
        rugbyUnionMatchStatus = new RugbyUnionMatchStatus();
    ...
    public int getPointsA() {
        return tryA * 5 + convA * 2 + dropGoalA * 3;
    }
    ...
    public int getPointsB() {
        return tryB * 5 + convB * 2 + dropGoalB * 3;
    }
     ...
     * @param matchFormat
     */
    public RugbyUnionMatchState(MatchFormat matchFormat) {
        super();
        this.matchFormat = (RugbyUnionMatchFormat) matchFormat;
        ballPosition = new BallPosition();
        matchPeriod = RugbyUnionMatchPeriod.PREMATCH;
    ...
    public RugbyUnionMatchStatus getRugbyUnionMatchStatus() {
        return rugbyUnionMatchStatus;
    }
    ...
    public RugbyUnionMatchPeriod getMatchPeriodInWhichLastGoalScored() {
        return matchPeriodInWhichLastGoalScored;
    }

    public void setMatchPeriodInWhichLastGoalScored(RugbyUnionMatchPeriod matchPeriodInWhichLastGoalScored) {
    ...
    public void setRugbyUnionMatchStatus(RugbyUnionMatchStatus rugbyUnionMatchStatus) {
    ...
    public void setElapsedTimeSecs(int elapsedTimeSecs, int overtimeNoIn) {
        ...
        matchPeriod = RugbyUnionMatchPeriod.IN_FIRST_HALF;
        if (elapsedTimeSecs == normalPeriodSecs) {
            this.elapsedTimeThisPeriodSecs = 0;
            matchPeriod = RugbyUnionMatchPeriod.AT_HALF_TIME;
        }
        if (elapsedTimeSecs > normalPeriodSecs) {
            this.elapsedTimeThisPeriodSecs = elapsedTimeSecs - normalPeriodSecs;
            matchPeriod = RugbyUnionMatchPeriod.IN_SECOND_HALF;
        }
        if (elapsedTimeSecs == 2 * normalPeriodSecs) {
            this.elapsedTimeThisPeriodSecs = 0;
            matchPeriod = RugbyUnionMatchPeriod.AT_FULL_TIME;
        }
        if (elapsedTimeSecs > 2 * normalPeriodSecs) {
            this.elapsedTimeThisPeriodSecs = elapsedTimeSecs - 2 * normalPeriodSecs;
            matchPeriod = RugbyUnionMatchPeriod.IN_EXTRA_TIME_FIRST_HALF;
        }
        if (elapsedTimeSecs == 2 * normalPeriodSecs + extraPeriodSecs) {
            this.elapsedTimeThisPeriodSecs = 0;
            matchPeriod = RugbyUnionMatchPeriod.AT_EXTRA_TIME_HALF_TIME;
        }
        if (elapsedTimeSecs > 2 * normalPeriodSecs + extraPeriodSecs) {
    ...
    public MatchIncidentResult updateStateForIncident(MatchIncident matchIncident,
        ...
        if (!((matchIncident instanceof RugbyUnionMatchIncident)
        ...
        if (matchIncident.getClass() == RugbyUnionMatchIncident.class) {

            TeamId teamId = ((RugbyUnionMatchIncident) matchIncident).getTeamId();

            switch (((RugbyUnionMatchIncident) matchIncident).getIncidentSubType()) {
                ...
                case PENALTY_START:
                    rugbyUnionMatchStatus.setRugbyUnionMatchStatus(RugbyUnionMatchStatusType.PENALTY, teamId);
                    break;

                case CONVERSION_START:
                    rugbyUnionMatchStatus.setRugbyUnionMatchStatus(RugbyUnionMatchStatusType.CONVERSION, teamId);
                    break;

                // case PENALTY_END:
                // rugbyUnionMatchStatus.resetRugbyUnionMatchStatus();
                // break;
                //
                // case CONVERSION_END:
                // rugbyUnionMatchStatus.resetRugbyUnionMatchStatus();
                ...
                case FIELD_POSITION_SETTING:
                    FieldPositionType fieldPosition = ((RugbyUnionMatchIncident) matchIncident).getFieldPosition();
                ...
                case SET_INJURY_TIME:
                    injuryTimeSecs = ((RugbyUnionMatchIncident) matchIncident).getInjuryTimeSecs();
                    break;
                ...
                case TRY:
                    ...
                    switch (teamId) {
                        case A:
                            if (!matchPeriod.equals(RugbyUnionMatchPeriod.IN_EXTRA_TIME_FIRST_HALF)
                                            && !matchPeriod.equals(RugbyUnionMatchPeriod.IN_EXTRA_TIME_SECOND_HALF))
                                normalTimePointsA += 5;
                            ...
                            if (matchPeriod.equals(RugbyUnionMatchPeriod.IN_FIRST_HALF))
                                pointsFirstHalfA += 5;
                            if (matchPeriod.equals(RugbyUnionMatchPeriod.IN_SECOND_HALF))
                                pointsSecondHalfA += 5;
                ...
                case CONVERSION_GOAL:
                    ...
                    checkIfPenatlyOrConversionGoalApproporate(RugbyUnionMatchIncidentType.CONVERSION_GOAL, teamId);
                    switch (teamId) {
                        case A:
                            if (!matchPeriod.equals(RugbyUnionMatchPeriod.IN_EXTRA_TIME_FIRST_HALF)
                                            && !matchPeriod.equals(RugbyUnionMatchPeriod.IN_EXTRA_TIME_SECOND_HALF))
                ...
                case CONVERSION_MISS:
                    checkIfPenatlyOrConversionGoalApproporate(RugbyUnionMatchIncidentType.CONVERSION_MISS, teamId);
                    break;
                case PENALTY_MISS:
                    checkIfPenatlyOrConversionGoalApproporate(RugbyUnionMatchIncidentType.PENALTY_MISS, teamId);
                    break;
                case PENALTY_GOAL:
                    elapsedTimeAtLastGoalSecs = elapsedTimeSecs;
                    matchPeriodInWhichLastGoalScored = matchPeriod;

                    checkIfPenatlyOrConversionGoalApproporate(RugbyUnionMatchIncidentType.PENALTY_GOAL, teamId);
                    switch (teamId) {
                        case A:
                            if (!matchPeriod.equals(RugbyUnionMatchPeriod.IN_EXTRA_TIME_FIRST_HALF)
                                            && !matchPeriod.equals(RugbyUnionMatchPeriod.IN_EXTRA_TIME_SECOND_HALF))
                ...
                case DROP_GOAL:
                    ...
                    switch (teamId) {
                        case A:
                            if (!matchPeriod.equals(RugbyUnionMatchPeriod.IN_SHOOTOUT)) {
                                ...
                                if (isShootoutSettled()) {
                                    ...
                                    matchPeriod = RugbyUnionMatchPeriod.MATCH_COMPLETED;
                                }
            ...
            switch (((ElapsedTimeMatchIncident) matchIncident).getIncidentSubType()) {
                    ...
                    switch (matchPeriod) {
                        ...
                        case AT_EXTRA_TIME_END:
                            ...
                            if (penaltiesPossible && (tryA + convA + dropGoalA == tryB + convB + dropGoalB)) {
                                matchPeriod = RugbyUnionMatchPeriod.IN_SHOOTOUT;
                                break;
                            } else {
                ...
                case SET_PERIOD_END:
                    switch (matchPeriod) {
                        ...
                        case IN_EXTRA_TIME_SECOND_HALF:
                            if (tryA + convA + dropGoalA == tryB + convB + dropGoalB) {
                                matchPeriod = RugbyUnionMatchPeriod.AT_EXTRA_TIME_END;
                            } else {
                                matchPeriod = RugbyUnionMatchPeriod.MATCH_COMPLETED;
                            }
                            previousPeriodGoalsA = currentPeriodPointsA;
    ...
    private void checkIfPenatlyOrConversionGoalApproporate(RugbyUnionMatchIncidentType rugbyUnionMatchIncidentType,
                    TeamId teamId) {
        // Penalty or conversion should only happens after penalty fault or a try.
        if (rugbyUnionMatchIncidentType == RugbyUnionMatchIncidentType.CONVERSION_GOAL
                        || rugbyUnionMatchIncidentType == RugbyUnionMatchIncidentType.CONVERSION_MISS) {
            if (rugbyUnionMatchStatus.getRugbyUnionMatchStatus() != RugbyUnionMatchStatusType.CONVERSION)
                throw new IllegalArgumentException("Conversion goal can not be happening now");
            rugbyUnionMatchStatus.resetRugbyUnionMatchStatus();
        } else if (rugbyUnionMatchIncidentType == RugbyUnionMatchIncidentType.PENALTY_GOAL
                        || rugbyUnionMatchIncidentType == RugbyUnionMatchIncidentType.PENALTY_MISS) {
            if (rugbyUnionMatchStatus.getRugbyUnionMatchStatus() != RugbyUnionMatchStatusType.PENALTY)
    ...
    public AlgoMatchState copy() {
        RugbyUnionMatchState cc = new RugbyUnionMatchState(matchFormat);
        cc.setEqualTo(this);
        return cc;
    ...
    public void setEqualTo(AlgoMatchState matchState) {
        ...
        try {
            this.ballPosition = (BallPosition) (((RugbyUnionMatchState) matchState).ballPosition).clone();
            this.rugbyUnionMatchStatus =
                            (RugbyUnionMatchStatus) (((RugbyUnionMatchState) matchState).rugbyUnionMatchStatus).clone();
        } catch (CloneNotSupportedException e) {
            e.printStackTrace();
        }
        this.setTryA(((RugbyUnionMatchState) matchState).getTryA());
        this.setTryB(((RugbyUnionMatchState) matchState).getTryB());

        this.setConvA(((RugbyUnionMatchState) matchState).getConvA());
        this.setConvB(((RugbyUnionMatchState) matchState).getConvB());

        this.setDropGoalA(((RugbyUnionMatchState) matchState).getDropGoalA());
        this.setDropGoalB(((RugbyUnionMatchState) matchState).getDropGoalB());

        this.setElapsedTimeSecs(((RugbyUnionMatchState) matchState).getElapsedTimeSecs(),
                        ((RugbyUnionMatchState) matchState).getOvertimeNo());
        this.setMatchPeriod(((RugbyUnionMatchState) matchState).getMatchPeriod());
        this.setTeamScoringLastGoal(((RugbyUnionMatchState) matchState).getTeamScoringLastGoal());
        /* update cards infor and sendoff info */
        this.setRedCardA(((RugbyUnionMatchState) matchState).getRedCardA());
        this.setRedCardB(((RugbyUnionMatchState) matchState).getRedCardB());

        this.tenMinsSinBinTimer_A.clear();
        this.tenMinsSinBinTimer_A = cloneIntegerList(((RugbyUnionMatchState) matchState).getTenMinsSinBinTimer_A());

        this.tenMinsSinBinTimer_B.clear();
        this.tenMinsSinBinTimer_B = cloneIntegerList(((RugbyUnionMatchState) matchState).getTenMinsSinBinTimer_B());
        this.setInjuryTimeSecs(((RugbyUnionMatchState) matchState).getInjuryTimeSecs());
        this.setTenMinsSinBinA(((RugbyUnionMatchState) matchState).getTenMinsSinBinA());
        this.setTenMinsSinBinB(((RugbyUnionMatchState) matchState).getTenMinsSinBinB());
        this.setShootOutGoalsA(((RugbyUnionMatchState) matchState).getShootOutGoalsA());
        this.setShootOutGoalsB(((RugbyUnionMatchState) matchState).getShootOutGoalsB());
        this.setShootOutTimeCounter(((RugbyUnionMatchState) matchState).getShootOutTimeCounter());
        this.setOvertimeNo(((RugbyUnionMatchState) matchState).getOvertimeNo());
        this.setGoalSequence(((RugbyUnionMatchState) matchState).getGoalSequence());
        this.setCurrentPeriodGoalsA(((RugbyUnionMatchState) matchState).getCurrentPeriodPointsA());
        this.setCurrentPeriodGoalsB(((RugbyUnionMatchState) matchState).getCurrentPeriodPointsB());
        this.setOvertimeNo(((RugbyUnionMatchState) matchState).getOvertimeNo());
        this.setNormalTimePointsA(((RugbyUnionMatchState) matchState).getNormalTimePointsA());
        this.setNormalTimePointsB(((RugbyUnionMatchState) matchState).getNormalTimePointsB());
        this.setElapsedTimeSecsFirstGoal(((RugbyUnionMatchState) matchState).getElapsedTimeSecsFirstGoal());
        this.setFiveMinsGoalsA(((RugbyUnionMatchState) matchState).getFiveMinsGoalsA());
        this.setFiveMinsGoalsB(((RugbyUnionMatchState) matchState).getFiveMinsGoalsB());
        this.setPreviousFiveMinsGoalsA(((RugbyUnionMatchState) matchState).getPreviousFiveMinsGoalsA());
        this.setPreviousFiveMinsGoalsB(((RugbyUnionMatchState) matchState).getPreviousFiveMinsGoalsB());
        this.setCurrentFiveMinsEnder(((RugbyUnionMatchState) matchState).getCurrentFiveMinsEnder());
        this.setPreviousFiveMinsEnder(((RugbyUnionMatchState) matchState).getPreviousFiveMinsEnder());
        this.setFiveMinsNo(((RugbyUnionMatchState) matchState).getFiveMinsNo());
        this.setCurrentPeriodTrysA(((RugbyUnionMatchState) matchState).getCurrentPeriodTrysA());
        this.setCurrentPeriodTrysB(((RugbyUnionMatchState) matchState).getCurrentPeriodTrysB());
    ...
    public MatchIncidentPrompt getNextPrompt() {
        ...
        if (matchPeriod == RugbyUnionMatchPeriod.MATCH_COMPLETED)
            matchIncidentPrompt = new MatchIncidentPrompt("Match finished");
        else if (matchPeriod == RugbyUnionMatchPeriod.PREMATCH || matchPeriod == RugbyUnionMatchPeriod.AT_HALF_TIME
                        || matchPeriod == RugbyUnionMatchPeriod.AT_FULL_TIME
                        || matchPeriod == RugbyUnionMatchPeriod.AT_EXTRA_TIME_HALF_TIME
                        || matchPeriod == RugbyUnionMatchPeriod.AT_EXTRA_TIME_END)
    ...
    public MatchIncident getMatchIncident(String response) {
         ...
         * 
         */
        RugbyUnionMatchIncidentType rugbyMatchIncidentType = null;
        ...
        switch (c) {
            case 'N':
                ...
                break;
            case 'I':
                rugbyMatchIncidentType = RugbyUnionMatchIncidentType.SET_INJURY_TIME;
                try {
            ...
            case 'M':
                rugbyMatchIncidentType = RugbyUnionMatchIncidentType.RESET_FIELD_BALL_INFO;
                break;
            ...
            case 'F':
                try {
                    ...
                    if (yard1.equalsIgnoreCase("M")) {
                        rugbyMatchIncidentType = RugbyUnionMatchIncidentType.FIELD_POSITION_SETTING;
            ...
            case 'C': {
                ...
                switch (s) {
                    case "S":
                        ...
                        if (i.equals("H")) {
                            rugbyMatchIncidentType = RugbyUnionMatchIncidentType.CONVERSION_START;
                            teamId = TeamId.A;
                        } else if (i.equals("A")) {
                            rugbyMatchIncidentType = RugbyUnionMatchIncidentType.CONVERSION_START;
                            teamId = TeamId.B;
                    ...
                    case "H":
                        rugbyMatchIncidentType = RugbyUnionMatchIncidentType.CONVERSION_GOAL;
                        teamId = TeamId.A;
                        break;
                    ...
                    case "M":
                        rugbyMatchIncidentType = RugbyUnionMatchIncidentType.CONVERSION_MISS;
            ...
            case 'P': {
                ...
                switch (s) {
                    case "H":
                        rugbyMatchIncidentType = RugbyUnionMatchIncidentType.PENALTY_START;
                        teamId = TeamId.A;
                        break;
                    ...
                    case "M":
                        rugbyMatchIncidentType = RugbyUnionMatchIncidentType.PENALTY_MISS;
                        teamId = TeamId.UNKNOWN;
                        break;
                }
                break;
            ...
            case 'H':
                if (response.length() == 1) {
                    ...
                    // if( rugbyUnionMatchStatus.getRugbyUnionMatchStatus().equals(RugbyUnionMatchStatusType.NORMAL))
                    rugbyMatchIncidentType = RugbyUnionMatchIncidentType.DROP_GOAL;

                } else {
                    String s = response.substring(1);
                    switch (s) {
                        case "P":
                            rugbyMatchIncidentType = RugbyUnionMatchIncidentType.PENALTY_GOAL;
                            teamId = TeamId.A;
                            break;
                        case "C":
                            rugbyMatchIncidentType = RugbyUnionMatchIncidentType.CONVERSION_GOAL;
            ...
            case 'B':
                try {
                    ...
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        rugbyMatchIncidentType = RugbyUnionMatchIncidentType.BALL_POSITION_SETTING;
                        teamId = TeamId.A;
                        break;
                    }
            ...
            case 'T':// hit a try
                try {
                    String teamCount = (response.substring(1));
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        rugbyMatchIncidentType = RugbyUnionMatchIncidentType.TRY;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
            ...
            case 'R':// red card and sin bins
                ...
                try {
                    ...
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        rugbyMatchIncidentType = RugbyUnionMatchIncidentType.TEN_MINS_PENALTY_START;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
                    ...
                    if (teamCount.equalsIgnoreCase("R")) { // away team yellow card
                        rugbyMatchIncidentType = RugbyUnionMatchIncidentType.RED_CARD;
            ...
            case 'Q':// terminate major sin bin
                ...
                try {
                    String teamCount = (response.substring(1));
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        rugbyMatchIncidentType = RugbyUnionMatchIncidentType.TEN_MINS_PENALTY_END;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
        ...
        if (rugbyMatchIncidentType != null) {
            if (rugbyMatchIncidentType != RugbyUnionMatchIncidentType.FIELD_POSITION_SETTING) {// &&rugbyMatchIncidentType
                                                                                               // !=
                                                                                               // RugbyUnionMatchIncidentType.CONVERSION_START&&rugbyMatchIncidentType
                                                                                               // !=
                                                                                               // RugbyUnionMatchIncidentType.PENALTY_START
                incident = new RugbyUnionMatchIncident(rugbyMatchIncidentType, incidentElapsedTimeSecs, teamId);
            } else {
                incident = new RugbyUnionMatchIncident(rugbyMatchIncidentType, incidentElapsedTimeSecs, fieldPosition);
            }
        } else {
    ...
    public LinkedHashMap<String, String> getAsMap() {
        ...
        map.put(goalsKey, String.format("%d-%d", tryA * 5 + convA * 2 + dropGoalA * 3,
                        tryB * 5 + convB * 2 + dropGoalB * 3));
        ...
        map.put(matchStatusKey, rugbyUnionMatchStatus.getRugbyUnionMatchStatus().toString());
    ...
    public boolean isMatchCompleted() {
        return (matchPeriod == RugbyUnionMatchPeriod.MATCH_COMPLETED);
    }
    ...
    public boolean isNormalTimeMatchCompleted() {
        return (!(matchPeriod == RugbyUnionMatchPeriod.PREMATCH || matchPeriod == RugbyUnionMatchPeriod.IN_FIRST_HALF
                        || matchPeriod == RugbyUnionMatchPeriod.IN_SECOND_HALF
                        || matchPeriod == RugbyUnionMatchPeriod.AT_HALF_TIME));
    }
    ...
    public boolean isPeriodCompleted() {
        return (matchPeriod == RugbyUnionMatchPeriod.AT_HALF_TIME || matchPeriod == RugbyUnionMatchPeriod.AT_FULL_TIME
                        || matchPeriod == RugbyUnionMatchPeriod.MATCH_COMPLETED);
    }
    ...
    public TeamId getMatchWinner() {
        if (!isMatchCompleted())
            ...
            TeamId teamId;
            if (tryA * 5 + convA * 2 + dropGoalA * 3 > tryB * 5 + convB * 2 + dropGoalB * 3)
                teamId = TeamId.A;
            else if (tryA * 5 + convA * 2 + dropGoalA * 3 == tryB * 5 + convB * 2 + dropGoalB * 3)
                teamId = TeamId.UNKNOWN;
            else
                teamId = TeamId.B;
            return teamId;
    ...
    public int hashCode() {
        ...
        result = prime * result + ((rugbyUnionMatchStatus == null) ? 0 : rugbyUnionMatchStatus.hashCode());
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        RugbyUnionMatchState other = (RugbyUnionMatchState) obj;
        ...
        if (rugbyUnionMatchStatus == null) {
            if (other.rugbyUnionMatchStatus != null)
                return false;
        } else if (!rugbyUnionMatchStatus.equals(other.rugbyUnionMatchStatus))
            return false;
    ...
    void incrementSimulationElapsedTime(int timeSliceSecs) {
        ...
        switch (matchPeriod) {
            ...
            case IN_SECOND_HALF:
                ...
                if (elapsedTimeThisPeriodSecs >= normalPeriodSecs + injuryTimeSecs) {
                    if (extraPeriodSecs == 0
                                    || tryA * 5 + convA * 2 + dropGoalA * 3 != tryB * 5 + convB * 2 + dropGoalB * 3) {
                        matchPeriod = RugbyUnionMatchPeriod.MATCH_COMPLETED;
                    } else {
    ...
    // public class RugbyUnionMatchStatus implements Cloneable {
    // @Override
    ...
    public boolean preMatch() {
        return (matchPeriod == RugbyUnionMatchPeriod.PREMATCH);
    }
    ...
    public SimpleMatchState generateSimpleMatchState() {
        RugbyUnionSimpleMatchState simpleMatchState = new RugbyUnionSimpleMatchState(this.preMatch(),
                        this.isMatchCompleted(), matchPeriod, elapsedTimeSecs, isClockRunning(), pointsFirstHalfA,
