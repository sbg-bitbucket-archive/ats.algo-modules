...
public class BandyMatchMarketsFactory extends MarketsFactory {

    /**
     * holds any facts about a simulation of a match that has just been played (via the Match class) that are need in
     * the calculation of the required statistics and that are not captured by the matchState object for Bandy if we
    ...
    BandyMatchMarketsFactory(BandyMatchState matchState) {
        ...
        matchWinner = new ThreeWayStatistic("FT:AXB", "Match Result", generateMarketFT, matchSequenceId, "A", "B", "D");

        puckLine = new TwoWayStatistic("FTOT:SPRD", "Match Winner", false, matchSequenceId, "A", "B");
        ...
        generateMarketFT = (matchState.getMatchPeriod() != BandyMatchPeriod.AT_HALF_TIME
                        && matchState.getMatchPeriod() != BandyMatchPeriod.AT_FULL_TIME
                        && matchState.getMatchPeriod() != BandyMatchPeriod.IN_EXTRA_TIME_FIRST_HALF
                        && matchState.getMatchPeriod() != BandyMatchPeriod.IN_EXTRA_TIME_SECOND_HALF
                        && matchState.getMatchPeriod() != BandyMatchPeriod.AT_EXTRA_TIME_HALF_TIME
                        && matchState.getMatchPeriod() != BandyMatchPeriod.AT_EXTRA_TIME_END
                        && matchState.getMatchPeriod() != BandyMatchPeriod.IN_SHOOTOUT
    ...
    void updateStats(BandyMatchState matchState, BandyMatchFacts matchFacts) {
