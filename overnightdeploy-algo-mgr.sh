...
mvn -Pcore clean install || error_exit
...
mvn -Pall clean install || error_exit
...
if [ "$AMELCOALGOBUILD" = "true" ]
then
mvn -Pcore deploy || error_exit
#mvn -DskipTests -Pcore deploy || error_exit
else
mvn -Pcore install || error_exit
...
if [ "$AMELCOALGOBUILD" = "true" ]
then
mvn -Palgo-repo deploy || error_exit
#mvn -Palgo-repo -DskipTests deploy || error_exit
else
mvn -Palgo-repo install || error_exit
