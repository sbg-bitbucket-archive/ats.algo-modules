...
/**
 * AlgoMatchState class for Fieldhockey
 * 
 * @author Jin
 *
 */
public class FieldhockeyMatchState extends AlgoMatchState {
    ...
    private FieldhockeyMatchPeriod matchPeriodInWhichLastGoalScored;

    private int elapsedTimeThisPeriodSecs; // recent MatchIncident;
    private final int normalPeriodSecs;
    private FieldhockeyMatchPeriod matchPeriod; // the state following the most
    private final FieldhockeyMatchFormat matchFormat;
     ...
     */
    public FieldhockeyMatchState() {
        this(new FieldhockeyMatchFormat());
        // super();
        // normalPeriodSecs = 70 * 30;
        ...
        // matchPeriod = FieldhockeyMatchPeriod.IN_SECOND_HALF;
        // extraPeriodSecs = 15 * 30;
     ...
     * @param matchFormat
     */
    public FieldhockeyMatchState(MatchFormat matchFormat) {
        super();
        this.matchFormat = (FieldhockeyMatchFormat) matchFormat;
    ...
    /**
     * Get the sequence ID for the present five minutes
     * 
     * @return
     */
    @JsonIgnore
    ...
    public void setElapsedTimeSecs(int elapsedTimeSecs, int overtimeNoIn) {
        ...
        if (twoHalvesFormat)
            matchPeriod = FieldhockeyMatchPeriod.IN_FIRST_HALF;
        else
            matchPeriod = FieldhockeyMatchPeriod.IN_FIRST_PERIOD;
        if (twoHalvesFormat) {
            if (elapsedTimeSecs == normalPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = 0;
                matchPeriod = FieldhockeyMatchPeriod.AT_HALF_TIME;
            }
            if (elapsedTimeSecs > normalPeriodSecs) {
            ...
            if (elapsedTimeSecs == 2 * normalPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = 0;
                matchPeriod = FieldhockeyMatchPeriod.AT_FULL_TIME;
            }
            if (elapsedTimeSecs > 2 * normalPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = elapsedTimeSecs - 2 * normalPeriodSecs;
                matchPeriod = FieldhockeyMatchPeriod.IN_EXTRA_TIME_FIRST_HALF;
            }
            if (elapsedTimeSecs == 2 * normalPeriodSecs + extraPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = 0;
                matchPeriod = FieldhockeyMatchPeriod.AT_EXTRA_TIME_HALF_TIME;
            }
            if (elapsedTimeSecs > 2 * normalPeriodSecs + extraPeriodSecs) {
            ...
            if (elapsedTimeSecs == normalPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = 0;
                matchPeriod = FieldhockeyMatchPeriod.AT_FIRST_PERIOD_END;
            }
            if (elapsedTimeSecs > normalPeriodSecs && elapsedTimeSecs < 2 * normalPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = elapsedTimeSecs - normalPeriodSecs;
                matchPeriod = FieldhockeyMatchPeriod.IN_SECOND_PERIOD;
            }
            if (elapsedTimeSecs == 2 * normalPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = 0;
                matchPeriod = FieldhockeyMatchPeriod.AT_HALF_TIME;
            }
            if (elapsedTimeSecs > 2 * normalPeriodSecs && elapsedTimeSecs < 3 * normalPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = elapsedTimeSecs - 2 * normalPeriodSecs;
                matchPeriod = FieldhockeyMatchPeriod.IN_THIRD_PERIOD;
            }
            if (elapsedTimeSecs == 3 * normalPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = 0;
                matchPeriod = FieldhockeyMatchPeriod.AT_THIRD_PERIOD_END;
            }
            if (elapsedTimeSecs > 3 * normalPeriodSecs && elapsedTimeSecs < 4 * normalPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = elapsedTimeSecs - 3 * extraPeriodSecs;
                matchPeriod = FieldhockeyMatchPeriod.IN_FOURTH_PERIOD;
            }
            ...
            if (elapsedTimeSecs > 4 * normalPeriodSecs && elapsedTimeSecs < 4 * normalPeriodSecs + extraPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = elapsedTimeSecs - 2 * normalPeriodSecs;
                matchPeriod = FieldhockeyMatchPeriod.IN_EXTRA_TIME_FIRST_HALF;
            }
            if (elapsedTimeSecs == 4 * normalPeriodSecs + extraPeriodSecs) {
                this.elapsedTimeThisPeriodSecs = 0;
                matchPeriod = FieldhockeyMatchPeriod.AT_EXTRA_TIME_HALF_TIME;
            }
            if (elapsedTimeSecs > 4 * normalPeriodSecs + extraPeriodSecs) {
    ...
    public MatchIncidentResult updateStateForIncident(MatchIncident matchIncident,
        ...
        if (matchIncident.getClass() == FieldhockeyMatchIncident.class) {

            TeamId teamId = ((FieldhockeyMatchIncident) matchIncident).getTeamId();

            switch (((FieldhockeyMatchIncident) matchIncident).getIncidentSubType()) {
                ...
                case GOAL:
                    ...
                    switch (teamId) {
                        case A:
                            if (!matchPeriod.equals(FieldhockeyMatchPeriod.IN_SHOOTOUT)) {
                                if (!matchPeriod.equals(FieldhockeyMatchPeriod.IN_EXTRA_TIME_FIRST_HALF) && !matchPeriod
                                                .equals(FieldhockeyMatchPeriod.IN_EXTRA_TIME_SECOND_HALF))
                                    normalTimeGoalsA++;
                                ...
                                if (matchPeriod.equals(FieldhockeyMatchPeriod.IN_FIRST_HALF))
                                    goalsFirstHalfA++;
                                if (matchPeriod.equals(FieldhockeyMatchPeriod.IN_SECOND_HALF))
                                    goalsSecondHalfA++;
                                ...
                                if (isShootoutSettled()) {
                                    ...
                                    matchPeriod = FieldhockeyMatchPeriod.MATCH_COMPLETED;
                                }
                            }
                            break;
                        case B:
                    ...
                    switch (matchPeriod) {
                        ...
                        case AT_EXTRA_TIME_END:
                            ...
                            if (penaltiesPossible && goalsA == goalsB) {
                                matchPeriod = FieldhockeyMatchPeriod.IN_SHOOTOUT;
                                break;
                            } else {
                                if (goalsA != goalsB) {
                                    matchPeriod = FieldhockeyMatchPeriod.MATCH_COMPLETED;
                ...
                case SET_PERIOD_END:
                    switch (matchPeriod) {
                        ...
                        case IN_SECOND_PERIOD:
                            ...
                            currentPeriodGoalsA = 0;
                            currentPeriodGoalsB = 0;
                            setElapsedTime(2 * normalPeriodSecs);
                            elapsedTimeAtLastMatchIncidentSecs = 2 * normalPeriodSecs;
                            setClockTimeOfLastElapsedTimeFromIncident();
                            break;
                        ...
                        case IN_THIRD_PERIOD:
                            ...
                            currentPeriodGoalsA = 0;
                            currentPeriodGoalsB = 0;
                            setElapsedTime(3 * normalPeriodSecs);
                            elapsedTimeAtLastMatchIncidentSecs = 3 * normalPeriodSecs;
                            setClockTimeOfLastElapsedTimeFromIncident();
                            break;
                        ...
                        case IN_FOURTH_PERIOD:
                            ...
                            elapsedTimeAtLastMatchIncidentSecs = 4 * normalPeriodSecs;
                            setClockTimeOfLastElapsedTimeFromIncident();
                            break;
                        ...
                        case IN_EXTRA_TIME_SECOND_HALF:
                            if (goalsA == goalsB) {
                                matchPeriod = FieldhockeyMatchPeriod.AT_EXTRA_TIME_END;
                            } else {
                                matchPeriod = FieldhockeyMatchPeriod.MATCH_COMPLETED;
                            }
                            previousPeriodGoalsA = currentPeriodGoalsA;
        ...
        switch (matchPeriod) {
            ...
            case IN_EXTRA_TIME_SECOND_HALF:
                if (twoHalvesFormat)
                    ...
                    elapsedTimeThisPeriodSecs = elapsedTimeSecs - 4 * normalPeriodSecs - extraPeriodSecs;
                }
                break;
            case IN_SHOOTOUT:
                if (twoHalvesFormat)
                    ...
                    elapsedTimeThisPeriodSecs = elapsedTimeSecs - 4 * normalPeriodSecs - extraPeriodSecs * 2;
                }
                break;
            case IN_FIRST_PERIOD:
                elapsedTimeThisPeriodSecs = elapsedTimeSecs - normalPeriodSecs;
    ...
    public AlgoMatchState copy() {
        FieldhockeyMatchState cc = new FieldhockeyMatchState(matchFormat);
        cc.setEqualTo(this);
        return cc;
    ...
    public void setEqualTo(AlgoMatchState matchState) {
        ...
        this.setGoalsA(((FieldhockeyMatchState) matchState).getGoalsA());
        this.setGoalsB(((FieldhockeyMatchState) matchState).getGoalsB());
        // this.setElapsedTimeSecs(((FieldhockeyMatchState) matchState).getElapsedTimeSecs(),
        // ((FieldhockeyMatchState) matchState).getOvertimeNo());
        this.setElapsedTimeSecs(((FieldhockeyMatchState) matchState).getElapsedTimeSecs());

        this.setElapsedTimeThisPeriodSecs(((FieldhockeyMatchState) matchState).getElapsedTimeThisPeriodSecs());

        this.setMatchPeriod(((FieldhockeyMatchState) matchState).getMatchPeriod());
        this.setTeamScoringLastGoal(((FieldhockeyMatchState) matchState).getTeamScoringLastGoal());
        /* update cards infor and sendoff info */
        this.twoMinsSinBinTimer_A.clear();
        this.twoMinsSinBinTimer_A = cloneIntegerList(((FieldhockeyMatchState) matchState).getTwoMinsSinBinTimer_A());

        this.twoMinsSinBinTimer_B.clear();
        this.twoMinsSinBinTimer_B = cloneIntegerList(((FieldhockeyMatchState) matchState).getTwoMinsSinBinTimer_B());

        this.fiveMinsSinBinTimer_A.clear();
        this.fiveMinsSinBinTimer_A = cloneIntegerList(((FieldhockeyMatchState) matchState).getFiveMinsSinBinTimer_A());

        this.fiveMinsSinBinTimer_B.clear();
        this.fiveMinsSinBinTimer_B = cloneIntegerList(((FieldhockeyMatchState) matchState).getFiveMinsSinBinTimer_B());

        this.setFiveMinsSinBinA(((FieldhockeyMatchState) matchState).getFiveMinsSinBinA());
        this.setFiveMinsSinBinB(((FieldhockeyMatchState) matchState).getFiveMinsSinBinB());
        this.setTwoMinsSinBinA(((FieldhockeyMatchState) matchState).getTwoMinsSinBinA());
        this.setTwoMinsSinBinB(((FieldhockeyMatchState) matchState).getTwoMinsSinBinB());
        this.setShootOutGoalsA(((FieldhockeyMatchState) matchState).getShootOutGoalsA());
        this.setShootOutGoalsB(((FieldhockeyMatchState) matchState).getShootOutGoalsB());
        this.setShootOutTimeCounter(((FieldhockeyMatchState) matchState).getShootOutTimeCounter());
        this.setOvertimeNo(((FieldhockeyMatchState) matchState).getOvertimeNo());
        this.setGoalSequence(((FieldhockeyMatchState) matchState).getGoalSequence());

        this.setCurrentPeriodGoalsA(((FieldhockeyMatchState) matchState).getCurrentPeriodGoalsA());
        this.setCurrentPeriodGoalsB(((FieldhockeyMatchState) matchState).getCurrentPeriodGoalsB());
        this.setOvertimeNo(((FieldhockeyMatchState) matchState).getOvertimeNo());
        this.setNormalTimeGoalsA(((FieldhockeyMatchState) matchState).getNormalTimeGoalsA());
        this.setNormalTimeGoalsB(((FieldhockeyMatchState) matchState).getNormalTimeGoalsB());
        this.setElapsedTimeSecsFirstGoal(((FieldhockeyMatchState) matchState).getElapsedTimeSecsFirstGoal());
        this.setFiveMinsGoalsA(((FieldhockeyMatchState) matchState).getFiveMinsGoalsA());
        this.setFiveMinsGoalsB(((FieldhockeyMatchState) matchState).getFiveMinsGoalsB());
        this.setPreviousFiveMinsGoalsA(((FieldhockeyMatchState) matchState).getPreviousFiveMinsGoalsA());
        this.setPreviousFiveMinsGoalsB(((FieldhockeyMatchState) matchState).getPreviousFiveMinsGoalsB());
        this.setCurrentFiveMinsEnder(((FieldhockeyMatchState) matchState).getCurrentFiveMinsEnder());
        this.setPreviousFiveMinsEnder(((FieldhockeyMatchState) matchState).getPreviousFiveMinsEnder());
        this.setFiveMinsNo(((FieldhockeyMatchState) matchState).getFiveMinsNo());
    ...
    public MatchIncidentPrompt getNextPrompt() {
        ...
        if (matchPeriod == FieldhockeyMatchPeriod.MATCH_COMPLETED)
            matchIncidentPrompt = new MatchIncidentPrompt("Match finished");
        else if (matchPeriod == FieldhockeyMatchPeriod.PREMATCH || matchPeriod == FieldhockeyMatchPeriod.AT_HALF_TIME
                        || matchPeriod == FieldhockeyMatchPeriod.AT_FULL_TIME
                        || matchPeriod == FieldhockeyMatchPeriod.AT_EXTRA_TIME_HALF_TIME
                        || matchPeriod == FieldhockeyMatchPeriod.AT_EXTRA_TIME_END)
    ...
    public MatchIncident getMatchIncident(String response) {
        ...
        FieldhockeyMatchIncidentType fieldhockeyMatchIncidentType = null;
        ...
        switch (c) {
            case 'N':
                ...
                break;
            case 'H':
                fieldhockeyMatchIncidentType = FieldhockeyMatchIncidentType.GOAL;
                teamId = TeamId.A;
                break;
            case 'A':
                fieldhockeyMatchIncidentType = FieldhockeyMatchIncidentType.GOAL;
            ...
            case 'G':// GREEN card
                ...
                try {
                    ...
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        fieldhockeyMatchIncidentType = FieldhockeyMatchIncidentType.TWO_MINS_PENALTY_START;
                        teamId = TeamId.A;
                        break;
                    }
            ...
            case 'Y':// red card 10 mins
                ...
                try {
                    String teamCount = (response.substring(1));
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        fieldhockeyMatchIncidentType = FieldhockeyMatchIncidentType.FIVE_MINS_PENALTY_START;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
            ...
            case 'Q':// terminate major sin bin
                ...
                try {
                    String teamCount = (response.substring(1));
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        fieldhockeyMatchIncidentType = FieldhockeyMatchIncidentType.FIVE_MINS_PENALTY_END;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
            ...
            case 'T':// terminate major sin bin
                ...
                try {
                    String teamCount = (response.substring(1));
                    if (teamCount.equalsIgnoreCase("H")) { // home team yellow card
                        fieldhockeyMatchIncidentType = FieldhockeyMatchIncidentType.TWO_MINS_PENALTY_END;
                        teamId = TeamId.A;
                        break;
                    }
                    if (teamCount.equalsIgnoreCase("A")) { // away team yellow card
        ...
        if (fieldhockeyMatchIncidentType != null) {
            incident = new FieldhockeyMatchIncident(fieldhockeyMatchIncidentType, incidentElapsedTimeSecs, teamId);
        } else {
    ...
    public boolean isMatchCompleted() {
        return (matchPeriod == FieldhockeyMatchPeriod.MATCH_COMPLETED);
    }
    ...
    public boolean isPeriodCompleted() {
        return (matchPeriod == FieldhockeyMatchPeriod.AT_HALF_TIME || matchPeriod == FieldhockeyMatchPeriod.AT_FULL_TIME
                        || matchPeriod == FieldhockeyMatchPeriod.MATCH_COMPLETED);
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        FieldhockeyMatchState other = (FieldhockeyMatchState) obj;
    ...
    void incrementSimulationElapsedTime(int timeSliceSecs) {
        ...
        switch (matchPeriod) {
            ...
            case IN_FIRST_HALF:
                if (!twoHalvesFormat) {
                    elapsedTimeThisPeriodSecs = elapsedTimeSecs;
                    if (elapsedTimeThisPeriodSecs >= 2 * normalPeriodSecs) {
                        elapsedTimeSecs = 2 * normalPeriodSecs; // reset counter
                                                                // to remove
                                                                // injury time
                        elapsedTimeThisPeriodSecs = 0;
                        matchPeriod = FieldhockeyMatchPeriod.IN_THIRD_PERIOD;
    ...
    public boolean preMatch() {
        return (matchPeriod == FieldhockeyMatchPeriod.PREMATCH);
    }
    ...
    public int secsLeftInCurrentPeriod() {
        ...
        switch (matchPeriod) {
            ...
            case IN_EXTRA_TIME_SECOND_HALF:
                ...
                break;
            case AT_FIRST_PERIOD_END:
                secs = normalPeriodSecs;
                break;
            case AT_THIRD_PERIOD_END:
                secs = 3 * normalPeriodSecs;
                break;
            case IN_FIRST_PERIOD:
                secs = normalPeriodSecs - elapsedTimeThisPeriodSecs;
                break;
            case IN_FOURTH_PERIOD:
                secs = 4 * normalPeriodSecs - elapsedTimeThisPeriodSecs;
                break;
            case IN_SECOND_PERIOD:
                secs = 2 * normalPeriodSecs - elapsedTimeThisPeriodSecs;
                break;
            case IN_THIRD_PERIOD:
                secs = 3 * normalPeriodSecs - elapsedTimeThisPeriodSecs;
                break;
            default:
                break;
        }
    ...
    public SimpleMatchState generateSimpleMatchState() {
        FieldhockeySimpleMatchState simpleMatchState = new FieldhockeySimpleMatchState(preMatch(), isMatchCompleted(),
                        getMatchPeriod(), elapsedTimeSecs, goalsA, goalsB, goalsFirstHalfA, goalsFirstHalfB,
                        goalsSecondHalfA, goalsSecondHalfB);
