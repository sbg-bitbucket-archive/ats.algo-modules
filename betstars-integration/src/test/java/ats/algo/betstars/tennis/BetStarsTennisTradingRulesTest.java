...
public class BetStarsTennisTradingRulesTest implements AlgoManagerPublishable {
    ...
    public BetStarsTennisTradingRulesTest() {
        /*
    ...
    public void test() {
         ...
         */
        assertEquals(SuspensionStatus.SUSPENDED_UNDISPLAY,
                        publishedMarkets.getMarkets().get("FT:TBIM_M").getMarketStatus().getSuspensionStatus());
        assertEquals("Not open in play",
                        publishedMarkets.getMarkets().get("FT:TBIM_M").getMarketStatus().getSuspensionStatusReason());
    ...
    private MarketPricesList getTestMarketPrices(boolean withNextGameWinner) {
        ...
        marketPricesList.put("TestSource2", m); // add second copy of prices to
