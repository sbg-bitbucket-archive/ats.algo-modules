...
public class CompetitionsList {
    ...
    public CompetitionsList(Competitions competitions) {
        competitionsList = new ArrayList<>(competitions.getCompetitions().size());
        for (Competition competition : competitions.values()) {
            competitionsList.add(new CompetitionsListEntry(competition.getEventID(), competition.getName()));
        }
    }
