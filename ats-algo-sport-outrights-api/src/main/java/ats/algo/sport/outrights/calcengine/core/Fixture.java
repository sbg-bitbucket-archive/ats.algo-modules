...
public class Fixture {
    ...
    private OutrightsFixtureStatus status;
    private boolean probsSourcedfromATS;
    private int goalsHome;
    private int goalsAway;
    private boolean fixtureSuspended;
    ...
    public Fixture(String date, FixtureType fixtureType, String fixtureID, String homeTeamID, String awayTeamID,
        ...
        this.status = OutrightsFixtureStatus.PRE_MATCH;
    ...
    /**
     * constructor for fixture which has already been played
     * 
     * @param date
     * @param fixtureType
     * @param fixtureID
     ...
     */
    public Fixture(String date, FixtureType fixtureType, String fixtureID, String homeTeamID, String awayTeamID,
                    String tag, boolean playedAtNeutralGround, int goalsHome, int goalsAway) {
        this(date, fixtureType, fixtureID, homeTeamID, awayTeamID, tag, playedAtNeutralGround);
        this.goalsHome = goalsHome;
        this.goalsAway = goalsAway;
        this.status = OutrightsFixtureStatus.COMPLETED;
    }
    ...
    public OutrightsFixtureStatus getStatus() {
        return status;
    }

    public void setStatus(OutrightsFixtureStatus status) {
        this.status = status;
    }
    ...
    public boolean isFixtureSuspended() {
        return fixtureSuspended;
    }

    public void setFixtureSuspended(boolean fixtureSuspended) {
        this.fixtureSuspended = fixtureSuspended;
    }

    public String winner() {
        if (status == OutrightsFixtureStatus.COMPLETED) {
    ...
    /**
     * updates fixture status and score based on the result
     * 
     * @param winningSelection
     */
    public void updateWithResult(String winningSelection) {
        PairOfIntegers score = PairOfIntegers.generateFromString(winningSelection);
        this.status = OutrightsFixtureStatus.COMPLETED;
        this.setGoalsHome(score.A);
        this.setGoalsAway(score.B);
    ...
    /**
     * infers the current score from the selections in the FT cs market. finds the lowest score that has a non zero
     * probability.
     * 
     * @param market
     */
    public void updateScore(Market market) {
        PairOfIntegers currentScore = new PairOfIntegers(Integer.MAX_VALUE, Integer.MAX_VALUE);
        for (Entry<String, Selection> e : market.getSelections().entrySet()) {
            ...
            double prob = e.getValue().getProb();
            if (prob > THRESHOLD) {
                if (score.A < currentScore.A)
                    currentScore.A = score.A;
                if (score.B < currentScore.B)
                    currentScore.B = score.B;
            }
        }
        this.goalsHome = currentScore.A;
        this.goalsAway = currentScore.B;

    }

    /**
     * results this fixture with the specified score
     * 
     * @param n
     * @param m
     */
    public void resultFixture(int n, int m) {
        this.setStatus(OutrightsFixtureStatus.COMPLETED);
        this.setGoalsHome(n);
        this.setGoalsAway(m);
    }
