...
public class RollerhockeyMatchMarketsFactory extends MarketsFactory {

    /**
     * holds any facts about a simulation of a match that has just been played (via the Match class) that are need in
     * the calculation of the required statistics and that are not captured by the matchState object for Rollerhockey if
    ...
    RollerhockeyMatchMarketsFactory(RollerhockeyMatchState matchState) {
        ...
        String fiveMinsSequenceId = matchState.getSequenceIdForFiveMins();
        ...
        generateMarketFT = (matchState.getMatchPeriod() != RollerhockeyMatchPeriod.AT_HALF_TIME
                        && matchState.getMatchPeriod() != RollerhockeyMatchPeriod.AT_FULL_TIME
                        && matchState.getMatchPeriod() != RollerhockeyMatchPeriod.IN_EXTRA_TIME_FIRST_HALF
                        && matchState.getMatchPeriod() != RollerhockeyMatchPeriod.IN_EXTRA_TIME_SECOND_HALF
                        && matchState.getMatchPeriod() != RollerhockeyMatchPeriod.AT_EXTRA_TIME_HALF_TIME
                        && matchState.getMatchPeriod() != RollerhockeyMatchPeriod.AT_EXTRA_TIME_END
                        && matchState.getMatchPeriod() != RollerhockeyMatchPeriod.IN_SHOOTOUT
    ...
    void updateStats(RollerhockeyMatchState matchState, RollerhockeyMatchFacts matchFacts) {
        ...
        if (creatNextPeriodMarket) {
            if (periodNo == 1) {
                periodGoalsA = matchState.getSecondHalfGoalsA();
                periodGoalsB = matchState.getSecondHalfGoalsB();
            }
        }
