...
public class PriceCalcRequestHandler {
    ...
    @Value("${urlForMqBroker:xyz}")
    private String urlSpringProperty;
    ...
    PriceCalcRequestHandler(Outrights outrights, String url) {

        if (url == null) {
            brokerUrl = "tcp://localhost:61616/"; // default
        } else {
            brokerUrl = url;
        }
    ...
    public void handlePriceCalcMessage(Message message) {
        ...
        logger.info("Price calc request received for eventId: " + request.getEventId() + ", uniqueRequestId: "
                        + request.getUniqueRequestId());

        if (request.getCalcRequestCause().equals(CalcRequestCause.MATCH_INCIDENT))
            logger.info("Outrights match incident:" + request.getMatchIncident().toString());
        ...
        logger.info("Price calc response issued for uniqueRequestId: " + response.getUniqueRequestId());
        if (response.isFatalError()) {
            logger.info("Fatal error flag in price calc response set to TRUE.");
            logger.info("Fatal error cause:" + response.getFatalErrorCause());
        } else {
            logger.info("Outrights watch list sent in response:" + response.getMatchEngineSavedState().toString());
            logger.info("Markets sent in response:" + response.getMarkets().toString());
        }
